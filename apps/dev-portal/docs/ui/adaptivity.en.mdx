...
<Alert type={'warning'} description={'Note that for mini-applications, the width of the window is not the size of the browser tab, but the size of the IFrame window in which the application is opened.'} showIcon/>
