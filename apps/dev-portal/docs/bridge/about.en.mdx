...
<Alert showIcon type="info" description={"If method \"<method-name>\" is called, the handler will receive event with type \"<method-name>Error\" in case of an error, and with type \"<method-name>Result\" in case of successful execution. This is common behavior for all Condo Bridge methods."}/>
