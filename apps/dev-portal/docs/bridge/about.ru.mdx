...
<Alert showIcon type="info" description={"При вызове метода \"<method-name>\" в случае ошибки в обработчик поступит событие с типом \"<method-name>Error\", а в случае успешного выполнения - с типом \"<method-name>Result\". Это общее поведение для всех методов Condo Bridge."}/>
