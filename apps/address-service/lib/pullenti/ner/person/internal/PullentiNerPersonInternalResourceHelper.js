...
const PullentiNerPersonInternalPropertiesResources = require("./properties/PullentiNerPersonInternalPropertiesResources");
...
class PullentiNerPersonInternalResourceHelper {
    ...
    static getBytes(name) {
        ...
        let names = PullentiNerPersonInternalPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                ...
                try {
                    let inf = PullentiNerPersonInternalPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = PullentiNerPersonInternalPropertiesResources.getStream(n); 
                    try {
    ...
    static getString(name) {
        let arr = PullentiNerPersonInternalResourceHelper.getBytes(name);
        if (arr === null) 
            return null;
...
module.exports = PullentiNerPersonInternalResourceHelper