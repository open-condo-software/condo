...
const PullentiNerOrgInternalPropertiesResources = require("./properties/PullentiNerOrgInternalPropertiesResources");
...
class PullentiNerOrgInternalResourceHelper {
    ...
    static getBytes(name) {
        ...
        let names = PullentiNerOrgInternalPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                ...
                try {
                    let inf = PullentiNerOrgInternalPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = PullentiNerOrgInternalPropertiesResources.getStream(n); 
                    try {
    ...
    static getString(name) {
        let arr = PullentiNerOrgInternalResourceHelper.getBytes(name);
        if (arr === null) 
            return null;
...
module.exports = PullentiNerOrgInternalResourceHelper