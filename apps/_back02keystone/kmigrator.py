...
def to_classname(value):
    ...
    """
    value = value.replace("public.", "").strip('_')
    return value
...
def _2_1_prepare_knex_jsons(ctx):
    try:
...
def _3_2_prepare_django_models(ctx):
...
def _3_3_prepare_django_migrations(ctx):
    repaired = set()
...
def _4_1_makemigrations(ctx):
    ...
    for item in (DJANGO_DIR / 'migrations').iterdir():
        ...
        bwd_sql = subprocess.check_output(cmd + ['--backwards']).decode('utf-8')
...
def main(command, keystoneEntryFile='./index.js'):
    ...
    try:
        ...
        _2_1_prepare_knex_jsons(ctx)
        _3_1_prepare_django(ctx)
        _3_2_prepare_django_models(ctx)
        _3_3_prepare_django_migrations(ctx)
