...
async function resolveConflicts ({ args, context }) {
    ...
    const developmentAppId = get(args, ['data', 'from', 'developmentApp', 'id'])
    const productionAppId = get(args, ['data', 'from', 'productionApp', 'id'])
    const app = await B2CApp.getOne(context, { id: apiAppId }, `id ${getEnvironmentalFieldsSelection(['exportId'])}`)
...
const ImportB2CAppService = new GQLCustomSchema('ImportB2CAppService', {
    types: [
        ...
        {
            access: true,
            type: 'enum ImportAppConflictPolicy { panic setNull delete }',
        },
        {
            access: true,
            type: 'input ImportB2CAppOptionsInput { info: Boolean!, builds: Boolean!, publish: Boolean!, accessRight: Boolean!, conflictPolicy: ImportAppConflictPolicy! }',
        },
        {
            access: true,
