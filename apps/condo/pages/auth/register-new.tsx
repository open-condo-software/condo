...
const MOCK_PHONE_TO_REPLACE = '+71234321234'
...
const ValidatePhoneForm = ({ onFinish, changeState }): React.ReactElement<IValidatePhoneFormProps>  => {
    ...
    const [phone, setPhone] = useState(formatPhone(MOCK_PHONE_TO_REPLACE))
    const [isPhoneVisible, setisPhoneVisible] = useState(false)
    const PhoneToggleLabel = isPhoneVisible ? intl.formatMessage({ id: 'Hide' }) : intl.formatMessage({ id: 'Show' })
    useEffect(() => {
        if (isPhoneVisible) {
            setPhone(formatPhone(MOCK_PHONE_TO_REPLACE))
        } else {
            const unHidden = formatPhone(MOCK_PHONE_TO_REPLACE)
                ...
                <Row gutter={[0, 24]} >
                    ...
                    <Col span={24} >
                        <Typography.Paragraph style={{ textAlign: 'left', marginTop: '32px' }}>
                            <a style={{ ...LINK_STYLE, fontSize: '12px', lineHeight: '20px' }} onClick={() => changeState('phoneInput')}>{ChangePhoneNumberLabel}</a>
                        </Typography.Paragraph>
                    </Col>
                </Row>
            </Form>
