...
const SettingsPage: PageComponentType = () => {
    ...
    const { hasSubscription, hasAvailablePlans, hasServicePlans, loading: subscriptionsLoading, subscriptionContext, hasSubscriptionsFeature } = useOrganizationSubscription()
    ...
    const availableTabs = useMemo(() => {
        if (!hasSubscription && isSubscriptionsEnabled && hasServicePlans) return [SETTINGS_TAB_SUBSCRIPTION]
    ...
    }, [hasSubscription, hasServicePlans, isSubscriptionsEnabled, canManageEmployeeRoles, isManagingCompany, canManageContactRoles, canManageMobileFeatureConfigsRoles, canManageMarketSettingRoles, acquiringIntegrationContext, acquiringIntegrationContextLoading])
    ...
    const settingsTabs: TabItem[] = useMemo(() => {
        ...
        if (!hasSubscription && isSubscriptionsEnabled && hasServicePlans) return [subscriptionTab]
