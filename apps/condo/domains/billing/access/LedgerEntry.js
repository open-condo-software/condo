...
async function canReadLedgerEntries ({ authentication: { item: user }, context, listKey }) {
...
async function canManageLedgerEntries ({ authentication: { item: user }, originalInput, operation, itemId, context, listKey }) {
    if (!user) return throwAuthenticationError()
    if (user.deletedAt) return false
    ...
    if (!organizationId && operation === 'create') {
        const ledgerId = get(originalInput, ['ledger', 'connect', 'id'])
        const ledger = ledgerId && await getById('TenantLedger', ledgerId)
        organizationId = get(ledger, 'organization')
    } else if (!organizationId && operation === 'update' && itemId) {
        const item = await getById('LedgerEntry', itemId)
