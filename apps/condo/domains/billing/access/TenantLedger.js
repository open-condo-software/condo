...
async function canReadTenantLedgers ({ authentication: { item: user }, context, listKey }) {
...
async function canManageTenantLedgers ({ authentication: { item: user }, originalInput, operation, itemId, context, listKey }) {
    if (!user) return throwAuthenticationError()
    if (user.deletedAt) return false
    ...
    if (!organizationId && operation === 'update' && itemId) {
        const item = await getById('TenantLedger', itemId)
...
module.exports = {
    canManageTenantLedgers,
    canReadTenantLedgers,
}