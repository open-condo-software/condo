...
describe('BillingReceipt', () => {
    ...
    describe('Webhook payload signature verification', () => {
        ...
        const webhookAppName = 'BillingReceiptWebhookReceiver'
        initTestExpressApp(webhookAppName, webhookApp)

        // Storage for received webhooks - keyed by payment ID
        const receivedWebhooks = {}
        // Storage for secrets - keyed by receipt ID (simulating what service would store)
        const storedSecrets = {}
        ...
        webhookApp.post('/webhook', (req, res) => {
            ...
            // Get the stored secret for this receipt
            const storedSecret = storedSecrets[receiptId]

            if (!storedSecret) {
                res.status(401).json({ error: 'Unknown receipt, no secret stored' })
                return
            }
        ...
        test('webhook is received and signature is verified by Express server', async () => {
            ...
            // 2. Create billing receipt with webhook URL - service user receives secret in response
            const [receipt] = await createTestBillingReceipt(admin, context, property, account, {
                paymentStatusChangeWebhookUrl: callbackUrl,
            })
            ...
            storedSecrets[receipt.id] = encryptionManager.decrypt(receipt.paymentStatusChangeWebhookSecret)

            // 4. Create a payment linked to this billing receipt
            const [payment] = await createTestPayment(admin, context.organization, receipt, acquiringContext, {
            ...
            expect(receivedWebhook.payload.receipt.__typename).toBe('BillingReceipt')
            expect(receivedWebhook.payload.receipt.id).toBe(receipt.id)

            // Clean up
            delete receivedWebhooks[payment.id]
            delete storedSecrets[receipt.id]
        })
    })
})
