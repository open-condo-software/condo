...
describe('BillingIntegrationOrganizationContext', () => {
    ...
    describe('Last report', () => {
        ...
        let admin
        beforeAll(async () => {
            const data = await makeContextWithOrganizationAndIntegrationAsAdmin()
            context = data.context
            admin = data.admin
        })
        test('can be saved without categories', async () => {
            const lastReport = {
                period: dayjs().format('YYYY-MM-01'),
                finishTime: new Date().toISOString(),
                totalReceipts: 1,
            }
            const [updatedContext] = await updateTestBillingIntegrationOrganizationContext(admin, context.id, { lastReport })
            expect(updatedContext.lastReport.period).toEqual(lastReport.period)
            expect(updatedContext.lastReport.finishTime).toEqual(lastReport.finishTime)
            expect(updatedContext.lastReport.totalReceipts).toEqual(lastReport.totalReceipts)
        })
        test('can be saved with categories', async () => {
            const lastReport = {
                period: dayjs().format('YYYY-MM-01'),
                finishTime: new Date().toISOString(),
                totalReceipts: 1,
                categories: [faker.datatype.uuid()],
            }
            const [updatedContext] = await updateTestBillingIntegrationOrganizationContext(admin, context.id, { lastReport })
            expect(updatedContext.lastReport.period).toEqual(lastReport.period)
            expect(updatedContext.lastReport.finishTime).toEqual(lastReport.finishTime)
            expect(updatedContext.lastReport.totalReceipts).toEqual(lastReport.totalReceipts)
            expect(updatedContext.lastReport.categories).toEqual(lastReport.categories)
        })
        test('can not have duplicated categories', async () => {
            ...
            await expectToThrowValidationFailureError(async () => {
                await updateTestBillingIntegrationOrganizationContext(admin, context.id, { lastReport })
            }, 'path:/categories msg:must NOT have duplicate items')
        })
    })
})