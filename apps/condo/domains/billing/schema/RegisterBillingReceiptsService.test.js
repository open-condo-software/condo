...
const { makeContextWithOrganizationAndIntegrationAsAdmin, BillingRecipient } = require('@condo/domains/billing/utils/testSchema')
...
describe('RegisterBillingReceiptsService', () => {
    ...
    test('Recipients should be synced using new flow', async () => {
        const payload = {
            tin: '7744001497',
        ...
        const testPayload = {
            ...
            receipts: [
                createRegisterBillingReceiptsPayload(payload),
            ],
        }
        const [processedReceipt] = await registerBillingReceiptsByTestClient(admin, testPayload)
        const [createdRecipient] = await BillingRecipient.getAll(admin, {
            id: get(processedReceipt[0], ['receiver', 'id']),
        })

        expect(createdRecipient).toBeDefined()
        expect(createdRecipient).toHaveProperty('id', processedReceipt[0].receiver.id)
