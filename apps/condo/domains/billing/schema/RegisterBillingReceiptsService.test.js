...
const { expectToThrowAccessDeniedErrorToResult } = require('@open-condo/keystone/test.utils')
...
describe('RegisterBillingReceiptsService', () => {
    ...
    beforeAll(async () => {
        ...
        clients.serviceUserWithoutAccessRights = await makeClientWithServiceUser()
        clients.serviceUserWithAccessRights = await makeClientWithServiceUser()
        await createTestBillingIntegrationAccessRight(clients.admin, billingIntegration, clients.serviceUserWithAccessRights['user'])
    ...
    describe('Access rights', () => {
        ...
        test('Service account can execute RegisterBillingReceiptsService', async () => {
            const [receipts] = await registerBillingReceiptsByTestClient(clients.serviceUserWithAccessRights, {
                context: { id: integration.billingContext.id },
                receipts: [],
            })
            expect(receipts).toHaveLength(0)
        ...
        test('Service account without Access Right can not execute RegisterBillingReceiptsService', async () => {
            await expectToThrowAccessDeniedErrorToResult(async () => {
                await registerBillingReceiptsByTestClient(clients.serviceUserWithoutAccessRights, {
                    context: { id: integration.billingContext.id },
                    receipts: [],
                })
            })
