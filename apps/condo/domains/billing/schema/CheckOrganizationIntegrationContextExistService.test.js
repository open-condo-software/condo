...
const { updateTestBillingIntegrationOrganizationContext } = require(
    '../utils/testSchema')
...
describe('CheckOrganizationIntegrationContextExistService', () => {
    ...
    test('user get isFound: false when execute with existing organization, but not ready billing', async () => {
        const client = await makeLoggedInClient()
    ...
    test('user get isFound: false when execute without existing organization', async () => {
        ...
        const { admin, context, organization } = await makeContextWithOrganizationAndIntegrationAsAdmin()
        await updateTestBillingIntegrationOrganizationContext(admin, context.id, { status: 'finished' })
