...
const { getRandomImplicitFeeDistribution } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { updateTestBillingRecipient } = require(
...
/**
 * Makes billing receipts payable:
 * 1. Get all recipients and make them approved (isApproved = true)
 * 2. Get all billing integration contexts,
 *    2.1 Find acquiringIntegrationContext
 *    2.2 Make it approved!
 * 3. Find all connected ServiceConsumers and make sure they are connected to the correct acquiring
 *
 * @param client
 * @param {array} billingReceipts
 * @param {array} approvedRecipientsIds a list of BillingRecipient ids which are approved (isApproved = true)
 * @return {Promise<*>}
 */
async function makeBillingReceiptsPayable (
    client,
    billingReceipts,
    approvedRecipientsIds = [],
) {

    for (const billingReceipt of billingReceipts) {
        const receiptId = billingReceipt.id
        if (!receiptId) {
            throw ('no billing receipt id!')
        }

        const recipientId = get(billingReceipt, ['receiver', 'id'])
        if (!recipientId) {
            throw ('no billing receipt receiver!')
        }

        // Step 1: Approve BillingRecipient
        await updateTestBillingRecipient(client, recipientId, {
            isApproved: true,
        })

        // Step 2: Approve BillingIntegrationOrganizationContext
        const contextId = get(billingReceipt, ['context', 'id'])
        if (!contextId) {
            throw ('no context id')
        }
        const billingAccountId = get(billingReceipt, ['account', 'id'])
        if (!billingAccountId) {
            throw ('no billing account id')
        }

        let acquiringIntegrationContextId
        const [ billingContext ] = await BillingIntegrationOrganizationContext.getAll(client, { id: contextId })

        const billingIntegration = get(billingContext, ['integration'])
        const organization = get(billingContext, ['organization'])

        const startDate = dayjs().add(-7, 'day').toISOString()
        const endDate = dayjs().add(7, 'day').toISOString()

        const existingAcquiringIntegrationContext = await AcquiringIntegrationContext.getOne(client, {
            organization: organization,
        })

        if (!existingAcquiringIntegrationContext) {
            const [acquiringIntegration] = await createTestAcquiringIntegration(client, [billingIntegration])
            ...
            acquiringIntegrationContextId = acquiringIntegrationContext.id
        } else {
            await updateTestAcquiringIntegrationContext(client, existingAcquiringIntegrationContext.id, {
                implicitFeeDistributionSchema: getRandomImplicitFeeDistribution(),
                paymentsAllowedFrom: startDate,
                paymentsAllowedTo: endDate,
            })
            acquiringIntegrationContextId = existingAcquiringIntegrationContext.id
        }

        // Step 3: Make sure that service consumer links
        const serviceConsumers = await ServiceConsumer.getAll(client, {
            billingAccount: { id: billingAccountId },
        })

        for (const serviceConsumer of serviceConsumers) {
            await updateTestServiceConsumer(client, serviceConsumer.id, {
                acquiringIntegrationContext: { connect: { id: acquiringIntegrationContextId } },
            })
        }
    }
...
describe('AllResidentBillingReceipts', () => {
    ...
    describe('resident should be able to get all needed fields', () => {
        ...
        beforeAll(async () => {
            ...
            await makeBillingReceiptsPayable(admin, [receipt])
    ...
    test('user with valid serviceAccount can read BillingReceipt without raw data', async () => {
        ...
        const [receipt2] = await createTestBillingReceipt(adminClient, context, billingProperty, billingAccount2)
        await makeBillingReceiptsPayable(adminClient, [receipt, receipt2])
    ...
    test('user with valid serviceAccount can read BillingReceipt without raw data even if there are other Resident with this service consumer', async () => {
        ...
        await makeBillingReceiptsPayable(adminClient, [receipt])
    ...
    test('user with valid serviceAccount can filter residentBillingReceipts by serviceConsumer', async () => {
        ...
        const [receipt1] = await createTestBillingReceipt(adminClient, contextA, billingPropertyA, billingAccountA)
        const [receipt2] = await createTestBillingReceipt(adminClient, contextA, billingPropertyA, billingAccountA2)
        ...
        const [receipt3] = await createTestBillingReceipt(adminClient, contextB, billingPropertyB, billingAccountB)

        await makeBillingReceiptsPayable(adminClient, [receipt1, receipt2, receipt3])
    ...
    test('user with valid multiple serviceAccounts can read all his BillingReceipts without raw data', async () => {
        ...
        await makeBillingReceiptsPayable(adminClient, [receipt1, receipt2])
    ...
    test('user with valid serviceAccount can read BillingReceipt without raw data with where query', async () => {
        ...
        await updateTestBillingReceipt(adminClient, receipt2.id, { toPay: '100.50' })

        const objs = await ResidentBillingReceipt.getAll(userClient, { toPay: '100.50' })
        expect(objs).toHaveLength(1)
        expect(objs[0].raw).toEqual(undefined)
        expect(objs[0].id).toEqual(receipt2.id)
    })
    ...
    test('user with valid serviceAccount gets result filtered by accountNumber and recipientId', async () => {
        ...
        await makeBillingReceiptsPayable(adminClient, [marchWaterReceipt, marchElectricityReceipt, aprilWaterReceipt])
    ...
    describe('paid field', () => {
        ...
        test('Test simple cases', async () => {
            ...
            await makeBillingReceiptsPayable(admin, [receiptWithSinglePayment, receiptWithNoPayments])
        ...
        test('Test case when receipt is paid and overwritten by billing integration.', async () => {
            ...
            await makeBillingReceiptsPayable(admin, [receiptWithSinglePayment])
            ...
            await makeBillingReceiptsPayable(admin, [newReceiptWithSinglePayment])
        ...
        test('Test case when receipt is duplicated in single period in single organization. e.g Housing and Water goes to different recipients', async () => {
            ...
            await createTestAcquiringIntegrationContext(organizationClient, organizationClient.organization, acquiringIntegration)
            await makeBillingReceiptsPayable(admin, [receiptForHousing, receiptForWater])
