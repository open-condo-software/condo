...
import { MultiPayment } from '@condo/domains/acquiring/utils/testSchema'
...
const { getRandomHiddenCard } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { updateTestMultiPayment } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { PAYMENT_DONE_STATUS } = require(
    '@condo/domains/acquiring/constants/payment')
const { updateTestPayment } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { registerMultiPaymentByTestClient } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { createTestAcquiringIntegrationContext } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { createTestAcquiringIntegration } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { createTestPayment } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { makePayer } = require('@condo/domains/acquiring/utils/testSchema')
const { makePayerAndPayments } = require(
    '@condo/domains/acquiring/utils/testSchema')
const { makeClientWithPropertyAndBilling } = require(
...
describe('AllResidentBillingReceipts', () => {
    ...
    describe('paid field', () => {
        ...
        test('When fetching allResidentBillingReceipts with multiple payments all sums match up', async () => {
            const UNIT_NAME = '22'

            const admin = await makeLoggedInAdminClient()

            // Prepare billing integration and receipt
            ...
            const [ residentReceiptBeforePayment ] = await ResidentBillingReceipt.getAll(residentClient, { serviceConsumer: { resident: { id: resident.id } } })

            // Mobile app tries to pay
            const registerMultiPaymentPayload = {
                consumerId: serviceConsumer.id,
                receiptsIds: residentReceiptBeforePayment.id,
            }
            ...
            const [ residentBillingReceiptAfterPayment ] = await ResidentBillingReceipt.getAll(residentClient, { serviceConsumer: { resident: { id: resident.id } } })

            expect(residentReceiptBeforePayment.paid).toEqual('0.00000000')
            expect(residentBillingReceiptAfterPayment.paid).toEqual('2000.00000000')
        })
    })
})