...
const BILLING_INTEGRATION_FIELDS = `{ name shortDescription gallery price detailedDescription logo { publicUrl } group developer partnerUrl appUrl contextDefaultStatus dataFormat ${BILLING_INTEGRATION_DATA_FORMAT_FIELDS} isHidden ${COMMON_FIELDS} skipNoAccountNotifications }`
...
const BILLING_INTEGRATION_ORGANIZATION_CONTEXT_FIELDS = `{ integration { id name appUrl billingPageTitle currencyCode dataFormat ${BILLING_INTEGRATION_DATA_FORMAT_FIELDS} skipNoAccountNotifications } organization { id tin name country } settings state status lastReport currentProblem { id title message } ${COMMON_FIELDS} }`
