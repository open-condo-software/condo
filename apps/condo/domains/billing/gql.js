...
const BILLING_INTEGRATION_ORGANIZATION_CONTEXT_FIELDS = `{ integration { id name appUrl checkAccountNumberUrl billingPageTitle setupUrl instruction instructionExtraLink connectedMessage connectedUrl uploadUrl uploadMessage extendsBillingPage billingPageIcon { publicUrl } currencyCode dataFormat ${BILLING_INTEGRATION_DATA_FORMAT_FIELDS} skipNoAccountNotifications } organization { id tin name country type } settings state status lastReport currentProblem { id title message } ${COMMON_FIELDS} }`
...
const BILLING_ACCOUNT_FIELDS = `{ context ${BILLING_INTEGRATION_ORGANIZATION_CONTEXT_FIELDS} importId property { id address addressKey } rentalUnit { id name unitType } number unitName unitType raw globalId meta fullName isClosed ownerType ${COMMON_FIELDS} }`
...
const BILLING_RECEIPT_COMMON_FIELDS = `context ${BILLING_INTEGRATION_ORGANIZATION_CONTEXT_FIELDS} importId property { id address addressKey } account { id number unitType unitName rentalUnit { id name unitType } fullName property { address } ownerType globalId isClosed } rentCharge { id billingMonth periodStart periodEnd dueDate amount currencyCode } period toPay printableNumber toPayDetails { ${BILLING_RECEIPT_TO_PAY_DETAILS_FIELDS} } ${BILLING_RECEIPT_SERVICE_FIELDS} charge formula balance recalculation privilege penalty paid receiver { id tin iec bic bankAccount isApproved } ${BILLING_RECEIPT_RECIPIENT_FIELDS} ${COMMON_FIELDS} category ${BILLING_CATEGORY_FIELDS} invalidServicesError`
...
const BILLING_RECEIPT_FIELDS = `{ ${BILLING_RECEIPT_COMMON_FIELDS} file { id file { id filename originalFilename publicUrl mimetype } controlSum } acquiringIntegrationId acquiringHostUrl canGroupReceipts currencyCode balanceUpdatedAt isPayable paymentStatusChangeWebhookUrl paymentStatusChangeWebhookSecret }`
...
const BillingReceiptForOrganization = generateGqlQueries('BillingReceipt', `{ 
        ...
        account { id number unitType unitName rentalUnit { id name unitType } fullName ownerType globalId isClosed }
