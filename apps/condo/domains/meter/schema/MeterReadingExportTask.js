        ...
        where: {
            ...
            hooks: {
                resolveInput: ({ resolvedData, fieldPath, operation }) => {
                    ...
                    const now = dayjs()
                    const twoMonthAgo = now.subtract(2, 'months')

                    const dateLte = get(findAllByKey(baseWhere, 'date_lte'), '0', now.toISOString())
                    let dateGte = get(findAllByKey(baseWhere, 'date_gte'), '0', twoMonthAgo.toISOString())

                    if (dayjs(dateLte).diff(dayjs(dateGte), 'months') > 2) {
                        dateGte = dayjs(dateLte).subtract(2, 'months')
                    }
