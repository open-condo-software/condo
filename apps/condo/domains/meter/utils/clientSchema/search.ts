...
const GET_METER_QUERY = gql`
    query getMeter($where: MeterWhereInput) {
        objs: allMeters(where: $where) {
            id
            number
            unitName
            organization {
...
const GET_METER_RESOURCE_QUERY = gql`
    query getMeterResource($id: ID, $organizationId: ID, $propertyId: ID, $unitName: String) {
        objs: allMeterResources(where: { id: $id }) {
            id
            number
            unitName
            organization {
