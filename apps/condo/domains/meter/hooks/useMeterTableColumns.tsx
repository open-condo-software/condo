...
const MeterReadingInput = ({ record, newMeterReadings, setNewMeterReadings }) => {
    ...
    const updateMeterReadingsValue = useCallback(
        (oldMeterReadings, newTariffValue) => {
    ...
    const meterReadingValueChangeHandler = useCallback(
        (e) => {
    ...
    return (
        <div style={INPUT_CONTAINER_STYLE} onClick={handleInputContainerClick}>
            <InputNumber
                ...
                min={0}
            />
            <div style={METER_READING_INPUT_ADDON_STYLE}>{meterResourceMeasure}</div>
        </div>
    )
}
...
export const useMeterTableColumns = () => {
    ...
    const meterResourceRenderer = useCallback(
        (record) => {
    ...
    const meterReadingRenderer = useCallback(
        (record) => (
            <MeterReadingInput record={record} newMeterReadings={newMeterReadings} setNewMeterReadings={setNewMeterReadings} />
        ),
        [newMeterReadings],
    )
    ...
    const tableColumns = useMemo(
        ...
        [
            ...
            LastReadingMessage,
            MeterNumberMessage,
            MeterReadingsMessage,
            PlaceMessage,
            ResourceMessage,
            SourceMessage,
            VerificationDateMessage,
            dateRenderer,
            meterReadingRenderer,
            meterResourceRenderer,
            textRenderer,
        ],
    )

    return useMemo(() => ({ tableColumns, newMeterReadings, setNewMeterReadings }), [newMeterReadings, tableColumns])
}
