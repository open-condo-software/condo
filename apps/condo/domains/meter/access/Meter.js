...
async function canReadMeters({ authentication: { item: user }, context }) {
    ...
    if (user.type === RESIDENT) {
        const availableMetersIds = await getAvailableResidentMeters(context, userId)
...
async function canManageMeters({ authentication: { item: user }, originalInput, operation, itemId, context }) {
    ...
    if (operation === 'create') {
        ...
        if (organizationIdFromMeter !== organizationIdFromProperty) return false

        return await checkPermissionInUserOrganizationOrRelatedOrganization(
            context,
            user.id,
            organizationIdFromMeter,
            'canManageMeters',
        )
