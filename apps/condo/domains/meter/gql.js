...
const METER_FIELDS = `{ number account billingAccountMeter { id } organization { id } property { id } unitName place resource { id } ${COMMON_FIELDS} }`
const Meter = generateGqlQueries('Meter', METER_FIELDS)

const METER_READING_FIELDS = `{ number date account billingAccountMeter { id } meter { id } property { id } organization { id } value sectionName floorName unitName client { id } clientName clientEmail clientPhone contact { id } source { id name } ${COMMON_FIELDS} }`
