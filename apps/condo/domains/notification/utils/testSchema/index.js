...
async function _internalSendResidentPhonesByTestClient(client, extraAttrs = {}) {
    if (!client) throw new Error('no client')
    const sender = { dv: 1, fingerprint: faker.random.alphaNumeric(8) }
    ...
    const { data, errors } = await client.mutate(_INTERNAL_SEND_RESIDENT_PHONES_MUTATION, { data: attrs })
    throwIfError(data, errors)
    return [data.result, attrs]
}
...
module.exports = {
    ...
    _internalSendResidentPhonesByTestClient,
