...
import { USER_MESSAGE_LIST_POLL_INTERVAL_IN_MS, UserMessageType } from '@condo/domains/notification/components/constants'
...
type UserPollUserMessagesType = (args: UserPollUserMessagesArgsType) => UserPollUserMessagesReturnType
...
export const usePollUserMessages: UserPollUserMessagesType = ({ queryMessagesVariables, skipQueryMessagesCondition }) => {
    const [isPollingTab, setIsPollingTab] = useState<boolean>(false)
    const [userMessages, setUserMessages] = useState<UserMessageType[]>([])

    console.log('isPollingTab', isPollingTab)
