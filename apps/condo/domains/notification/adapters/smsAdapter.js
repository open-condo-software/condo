...
class SMSAdapter {
    ...
    async checkIsAvailable() {
        const result = await this.adapter.checkIsAvailable()
        return result
    }
...
class SmsRu {
    ...
    constructor() {
        ...
        this.isConfigured = validateConfig(config, ['api_url', 'token', 'from'])
    ...
    async checkIsAvailable() {
        const result = await fetch(`${this.api_url}/auth/check`, {
            method: 'POST',
    ...
    async send({ phone, message }, extendedParams = {}) {
        ...
        const result = await fetch(`${this.api_url}/sms/send`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
class SmsCRu {
    ...
    constructor() {
        ...
        this.isConfigured = validateConfig(config, ['api_url', 'login', 'password', 'sender'])
        this.api_url = config.api_url
    ...
    async checkIsAvailable() {
        ...
        const result = await fetch(`${this.api_url}/sys/balance.php`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: Object.entries(body)
                .map(([name, value]) => `${name}=${encodeURI(value)}`)
    ...
    async send({ phone, message }, extendedParams = {}) {
        ...
        const result = await fetch(`${this.api_url}/sys/send.php`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: Object.entries(body)
                .map(([name, value]) => `${name}=${encodeURI(value)}`)
