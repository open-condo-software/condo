        ...
        describe('called by Admin', () => {
            ...
            describe('Required fields in different push types', () => {
                it('CANCELED_CALL_MESSAGE_PUSH_TYPE has required fields in message', async () => {
                    ...
                    expect(transportMeta.transport).toEqual(PUSH_TRANSPORT)
                    expect(transportMeta.messageContext.baseData.messageCreatedAt).toEqual(message.createdAt)
                    expect(transportMeta.messageContext.baseData.voipIncomingCallId).toEqual(metaData['voipIncomingCallId'])
                    expect(transportMeta.messageContext.baseData.residentId).toEqual(metaData['residentId'])
                    expect(transportMeta.messageContext.baseData.B2CAppName).toEqual(metaData['B2CAppName'])
                    expect(transportMeta.messageContext.baseData.B2CAppContext).toEqual(metaData['B2CAppContext'])
                    expect(transportMeta.messageContext.baseData.B2CAppId).toEqual(metaData['B2CAppId'])
                })
            })
