...
describe('SyncRemoteClientService', () => {
    ...
    describe('Push tokens cleaning in case of new deviceKey', () => {
        ...
        test('normal behaviour: remote client has deviceKey and it is passed', async () => {
            ...
            expect(pushTokensV1[0]).toEqual(expect.objectContaining({
                isPush: pushTokenFirst.isPush,
                isVoIP: pushTokenFirst.isVoIP,
                provider: pushTokenFirst.transport,
            }))
            ...
            const pushTokenCreatedBefore = pushTokensV2.find(pushToken => pushToken.id === pushTokensV1[0].id)
            const [pushTokenCreatedNow] = pushTokensV2.filter(pushToken => pushToken !== pushTokenCreatedBefore)
            expect(pushTokenCreatedNow).toEqual(expect.objectContaining({
                isPush: pushTokenSecond.isPush,
                isVoIP: pushTokenSecond.isVoIP,
                provider: pushTokenSecond.transport,
            }))
            expect(pushTokenCreatedBefore).toEqual(expect.objectContaining(pushTokensV1[0]))
            ...
            expect(pushTokensV3).toEqual(expect.arrayContaining([
                expect.objectContaining(omit(pushTokenCreatedNow, 'remoteClient')),
            ]))
        })
        ...
        test('old behaviour: no deviceKey in remote client and no device key provided', async () => {
            ...
            const remoteClientPushToken = await RemoteClientPushToken.getOne(admin, { remoteClient: { id: remoteClientId } })
            ...
            const remoteClientAfterUpdate = await RemoteClientWithDeviceKey.getOne(admin, { id: remoteClientAfterUpdateId })
            ...
            const remoteClientPushTokenAfterUpdate = await RemoteClientPushToken.getOne(admin, { remoteClient: { id: remoteClientId } })
