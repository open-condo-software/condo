...
const mockResponseFromFintechApi = {
    advanceAcceptances: () => {
        throw Error('Please, implement a mock for this function, before using it')
    },
}
...
jest.mock('../SbbolFintechApi', () => ({
    initSbbolFintechApi: jest.fn().mockImplementation(() => ({
        fetchAdvanceAcceptances: () => {
            return mockResponseFromFintechApi.advanceAcceptances()
        },
    })),
}))
...
describe('syncSubscriptions', () => {
    ...
    describe('Fintech API returns active offer for current organization', () => {
        ...
        it('creates new subscription and stops already active one', async () => {
            ...
            const [initialDefaultTrialSubscription] = await ServiceSubscription.getAll(
                adminContext,
                {
            ...
            const subscriptions = await ServiceSubscription.getAll(
                adminContext,
                {
                    organization: { id: organization.id },
                },
            ...
            expect(
                dayjs(trialSubscriptionAfterSync.finishAt).isBefore(dayjs(initialDefaultTrialSubscription.finishAt)),
            ).toBeTruthy()
            ...
            expect(dayjs(sbbolSubscriptionAfterSync.finishAt).diff(dayjs(sbbolSubscriptionAfterSync.startAt), 'day')).toEqual(
    ...
    describe('Fintech API returns not active offer for current organization', () => {
        it('deactivates current sbbol subscription', async () => {
            ...
            const [currentSbbolSubscription] = await ServiceSubscription.getAll(
                adminContext,
                {
            ...
            const [changedSbbolSubscription] = await ServiceSubscription.getAll(
                adminContext,
                {
                    type: SUBSCRIPTION_TYPE.SBBOL,
                    organization: { id: organization.id },
    ...
    describe('Fintech API returns not active offer for another organization', () => {
        it('does nothing with subscription of current organization', async () => {
            ...
            const [sbbolSubscriptionAfterSync] = await ServiceSubscription.getAll(
                adminContext,
                {
                    type: SUBSCRIPTION_TYPE.SBBOL,
                    organization: { id: organization.id },
