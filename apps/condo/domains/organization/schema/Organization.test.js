...
const { createTestBillingIntegrationOrganizationContext, createTestBillingIntegration, makeClientWithIntegrationAccess, makeContextWithOrganizationAndIntegrationAsAdmin } = require('@condo/domains/billing/utils/testSchema')
...
describe('Check read access to organization from service user with billing integration',  () => {

    it('can get organization if organization has billing context', async () => {
        const admin = await makeLoggedInAdminClient()
        const serviceUserClient = await makeClientWithIntegrationAccess(admin)
        const [organization] = await registerNewOrganization(admin)
        await createTestBillingIntegrationOrganizationContext(admin, organization, serviceUserClient.integration)
    ...
    it('can not get organization if organization has no billing context', async () => {
        const admin = await makeLoggedInAdminClient()
        const serviceUserClient = await makeClientWithIntegrationAccess(admin)
        const [organization] = await registerNewOrganization(admin)
