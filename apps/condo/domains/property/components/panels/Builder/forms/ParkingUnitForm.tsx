...
const ParkingUnitForm: React.FC<IPropertyMapModalForm> = ({ builder, refresh, setDuplicatedUnitIds }) => {
    ...
    const NameLabel = intl.formatMessage({ id: 'property.parkingUnit.name' })
    ...
    const SectionTitlePrefix = intl.formatMessage({ id: 'property.select.option.parking' })
    const RenameNextParkingUnitsLabel = intl.formatMessage({ id: 'property.modal.renameNextParkingUnits' })
    const ParkingPlaceErrorLabel = intl.formatMessage({ id: 'property.warning.modal.sameParkingPlaceNamesErrorMsg' })
