...
export const BuildingPanelView: React.FC<IBuildingPanelViewProps> = ({ map }) => {
    ...
    return <PropertyMapView builder={Map} refresh={refresh} />
}
                    ...
                    <Col span={24} style={CHESS_SCROLL_HOLDER_STYLE}>
                        ...
                        >
                            ...
                            {builder.sections.map((section) => (
                                <MapSectionContainer key={section.id} visible={builder.isSectionVisible(section.id)}>
                                            ...
                                            return (
                                                <div key={floorInfo.id} style={FLOOR_CONTAINER_STYLE}>
                                                    {floorInfo.units.map((unit) => {
                                                        return (
                                                            <UnitButton key={unit.id} noninteractive>
                                                                {unit.label}
                                                            </UnitButton>
                                                        )
                                                    })}
                                    ...
                                    <UnitButton secondary style={UNIT_BUTTON_SECTION_STYLE} disabled>
                                        {section.name}
                                    </UnitButton>
                                </MapSectionContainer>
                            ))}
