...
import { EmptyBuildingBlock, EmptyFloor, BuildingAxisY, BuildingChooseSections } from './BuildingPanelCommon'
import { Button } from '@condo/domains/common/components/Button'
import { UnitButton } from '@condo/domains/property/components/panels/Builder/UnitButton'
import { MapEdit, BuildingMap, BuildingUnit, BuildingSection } from './MapConstructor'
...
export const BuildingPanelEdit: React.FC<IBuildingPanelEditProps> = ({
    mapValidationError,
    map,
    updateMap: updateFormField,
    handleSave,
    address,
}) => {
    const intl = useIntl()
        ...
        <FullscreenWrapper mode={'edit'} className={isFullscreen ? 'fullscreen' : ''}>
            <FullscreenHeader edit={true}>
                <Row style={{ paddingBottom: '39px', marginRight: '36px' }}>
                    {address && (
                        <Col flex={0} style={{ marginTop: '10px' }}>
                            <b>{address}</b>
                        </Col>
                    )}
                    <Col style={{ marginLeft: 'auto' }}>
                        <Button key="submit" onClick={handleSave} type="sberPrimary" disabled={!address}>
                            {SaveLabel}
                        </Button>
                    </Col>
                </Row>
...
const EditSectionForm: React.FC<IEditSectionFormProps> = ({ Builder, refresh }) => {
    ...
    return (
        <>
            ...
            <Col flex={0}>
                <Button secondary onClick={updateSection} type="sberPrimary" style={{ marginTop: '30px' }}>
                    {' '}
                    {SaveLabel}{' '}
                </Button>
                <Button
