...
import { EmptyBuildingBlock, EmptyFloor, BuildingAxisY, BuildingChooseSections, MapSectionContainer } from './BuildingPanelCommon'
import { Button } from '@condo/domains/common/components/Button'
import { UnitButton } from '@condo/domains/property/components/panels/Builder/UnitButton'
import { MapEdit } from './MapConstructor'
import { BuildingMap, BuildingUnit, BuildingSection } from '@app/condo/schema'
...
export const BuildingPanelEdit: React.FC<IBuildingPanelEditProps> = (props) => {
    ...
    const quickSave = Property.useUpdate({}, () =>
    ...
    const quickSaveCallback = useCallback(
        (event) => {
        ...
        <FullscreenWrapper mode={'edit'} className="fullscreen">
            <FullscreenHeader edit={true}>
                <Row css={TopRowCss} justify="space-between">
                    ...
                    <Col flex={0}>
                        <Dropdown trigger={DROPDOWN_TRIGGER} overlay={menuOverlay} css={DropdownCss} mouseEnterDelay={0}>
                            <Button type="sberBlack">
                                {AddElementTitle}
                                <DownOutlined />
                            </Button>
                        </Dropdown>
                ...
                <BuildingPanelTopModal
                    visible={!isNull(mode)}
                    title={!isNull(mode) ? intl.formatMessage({ id: `pages.condo.property.modal.title.${mode}` }) : null}
                ...
                >
                    {useMemo(
                        ...
                        [mode, mapEdit, refresh],
                    )}
                </BuildingPanelTopModal>
            </FullscreenHeader>
            <Row align="middle" style={{ height: '100%' }}>
                {
                    <ChessBoard builder={mapEdit} refresh={refresh} scrollToForm={scrollToForm} isFullscreen>
                        <Space size={20} align={'center'}>
                            <Button key="submit" onClick={handleSave} type="sberDefaultGradient" disabled={!address}>
                                {SaveLabel}
                            </Button>
                            <Button key="cancel" onClick={onCancel} type="sberDefaultGradient" secondary>
                                {CancelLabel}
                            </Button>
                            {mapValidationError ? (
    ...
    useEffect(() => {
        ...
        if (shouldMoveContainer) {
            ...
            if (builder.editMode === 'addSection') {
                ...
                const lastSectionUnitIds = last(builder.sections).floors.flatMap((floor) => floor.units.map((unit) => unit.id))
            ...
            {builder.isEmpty ? (
                ...
                <Col span={24} style={CHESS_SCROLL_HOLDER_STYLE}>
                    <ScrollContainer
                        ...
                        innerRef={container}
                    >
                        {!isEmpty(builder.sections) && <BuildingAxisY floors={builder.possibleChosenFloors} />}
                                ...
                                >
                                    {builder.possibleChosenFloors.map((floorIndex) => {
                                        ...
                                        if (floorInfo && floorInfo.units.length) {
                                            ...
                                            return <EmptyFloor key={`empty_${section.id}_${floorIndex}`} />
                                        }
                                    })}
                                </PropertyMapSection>
                            )
        ...
        <Row gutter={MODAL_FORM_ROW_GUTTER} css={FormModalCss}>
            <Col span={24}>
                <Select value={copyId} onSelect={setCopyId} disabled={builder.isEmpty}>
                    <Select.Option key={'create'} value={null}>
                        {CreateNewLabel}
                    </Select.Option>
                    {builder.map.sections
                        .filter((section) => !section.preview)
                        .map((section) => (
                            <Select.Option key={`copy-${section.id}`} value={section.id}>
                                {CopyLabel}
                                {section.name}
                            </Select.Option>
                        ))}
            ...
            <Col span={24}>
                <Space direction={'vertical'} size={32}>
                    ...
                    <Row gutter={MODAL_FORM_ROW_BUTTONS_GUTTER}>
                        <Col span={24}>
                            <Button secondary onClick={applyChanges} type="sberDefaultGradient" disabled={!(floor && section)}>
                                {' '}
                                {SaveLabel}{' '}
                            </Button>
                        </Col>
                        {mode === 'editUnit' && (
                            <Col span={24}>
                                <Button secondary onClick={deleteUnit} type="sberDangerGhost" icon={<DeleteFilled />}>
                                    {DeleteLabel}
                                </Button>
                            </Col>
                        )}
