...
const Property = new GQLListSchema('Property', {
    ...
    fields: {
        ...
        totalTicketsCount: {
            schemaDoc: 'Counter for all tickets',
            type: Virtual,
            resolver: async (item, _, context) => {
                const { data, errors } = await context.executeGraphQL({
                    query: Ticket.GET_ALL_OBJS_WITH_COUNT_QUERY,
                    variables: {
                        where: {
                            property: {
                                id: item.id,
                ...
                if (errors) {
                    console.error('Error while fetching virtual field totalTickets', errors)
                    return 0
                }

                return get(data, ['meta', 'count'])
            },
        },
    },
    ...
    queries: [
        {
            ...
            resolver: async (parent, args, context = {}) => {
                ...
                quickSort(
                    ...
                    totalObjects,
                    isAscendSort,
                    (property) => property.totalTicketsCount
                )
