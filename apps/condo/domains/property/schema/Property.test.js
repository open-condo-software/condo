import { catchErrorFrom, expectToThrowAccessDeniedErrorToObj, expectToThrowAuthenticationErrorToObj, expectToThrowAuthenticationErrorToObjects } from '@condo/domains/common/utils/testSchema'
...
describe('Property', () => {
    ...
    test('employee from "from" related organization: can read property', async () => {
        ...
        const properties = await Property.getAll(clientFrom, { id: propertyTo1.id })
        expect(properties).toHaveLength(1)
    })

    test('employee from "to" related organization: can\'t read property from "from"', async () => {
        const admin = await makeLoggedInAdminClient()
        const [organizationFrom] = await createTestOrganization(admin)
        const [role] = await createTestOrganizationEmployeeRole(admin, organizationFrom)
        const clientFrom = await makeClientWithNewRegisteredAndLoggedInUser()
    ...
    test('user: can\'t read not his own properties', async () => {
        const admin = await makeLoggedInAdminClient()
        const [organizationFrom] = await createTestOrganization(admin)
        const [role] = await createTestOrganizationEmployeeRole(admin, organizationFrom)
        const clientFrom = await makeClientWithNewRegisteredAndLoggedInUser()
        ...
        const properties = await Property.getAll(user, { id: propertyFrom.id })
        expect(properties).toHaveLength(0)
    })
})
