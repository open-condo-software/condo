...
describe('AllMiniAppsService', () => {
    ...
    describe('Data shown', () => {
        ...
        test('Miniapps must be sorted by descending displayPriority, then by descending createdAt', async () => {
            const [data] = await allMiniAppsByTestClient(employee, organization)
            expect(data).toBeDefined()
    ...
    describe('Filtering', () => {
        ...
        test('Filtering by category must work', async () => {
            const [otherApps] = await allMiniAppsByTestClient(employee, organization, { where: { category: OTHER_CATEGORY } })
            expectFilterSeparation(otherApps,
                [connectedCommonApp, nonConnectedApp],
                [connectedPartialApp, nonFinishedApp],
            )
            const [smartHomeApps] = await allMiniAppsByTestClient(employee, organization, { where: { category: SMART_HOME_CATEGORY } })
        ...
        test('Filtering by id_not must work', async () => {
            const [filteredApps] = await allMiniAppsByTestClient(employee, organization, { where: { id_not: connectedCommonApp.id } })
        ...
        test('Combined filters must work', async () => {
            ...
            const [excludedFilteredApps] = await allMiniAppsByTestClient(employee, organization, { where: { connected: true, accessible: true, id_not: connectedCommonApp.id } })
