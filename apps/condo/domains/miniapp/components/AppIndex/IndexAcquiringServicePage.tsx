...
interface IndexAcquiringServicePageProps {
    id: string,
}

export const IndexAcquiringServicePage: React.FC<IndexAcquiringServicePageProps> = ({ id }) => {
    const intl = useIntl()
    ...
    useEffect(() => {
        if (integration && !contextLoading && !contextError && !context && !integration.appUrl) {
            router.push(`/services/${id}/about?type=${ACQUIRING_APP_TYPE}`)
        }
