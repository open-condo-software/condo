...
const { PAYMENT_INIT_STATUS, PAYMENT_DONE_STATUS } = require('@condo/domains/acquiring/constants/payment')
const {
...
describe('Invoice', () => {
    ...
    describe('Webhook payload signature verification', () => {
        // Set up Express server to receive webhooks
        // nosemgrep: javascript.express.security.audit.express-check-csurf-middleware-usage.express-check-csurf-middleware-usage
        const webhookApp = express()
        webhookApp.use(express.json())
        webhookApp.use(express.text({ type: 'application/json' }))

        const webhookAppName = 'InvoiceWebhookReceiver'
        initTestExpressApp(webhookAppName, webhookApp)

        // Storage for received webhooks - keyed by payment ID
        const receivedWebhooks = {}
        // Storage for secrets - keyed by invoice ID (simulating what service would store)
        const storedSecrets = {}

        // Webhook endpoint that verifies signature and stores the payload
        webhookApp.post('/webhook', (req, res) => {
            const signature = req.headers['x-webhook-signature']
            const webhookId = req.headers['x-webhook-id']
            const rawBody = typeof req.body === 'string' ? req.body : JSON.stringify(req.body)

            // Parse payload to get payment ID for lookup
            let payload
            try {
            ...
            const storedSecret = storedSecrets[invoiceId]

            if (!storedSecret) {
                res.status(401).json({ error: 'Unknown invoice, no secret stored' })
                return
            }
            ...
            // Store the received webhook for test assertions
            receivedWebhooks[paymentId] = {
                payload,
                signature,
                webhookId,
                signatureValid,
        ...
        test('webhook is received and signature is verified by Express server', async () => {
            const { baseUrl } = getTestExpressApp(webhookAppName)
            const callbackUrl = `${baseUrl}/webhook`

            // 1. Register webhook URL in whitelist
            await createTestPaymentStatusChangeWebhookUrl(adminClient, {
                url: callbackUrl,
                isEnabled: true,
            })

            // 2. Create invoice with webhook URL - service user receives secret in response
            const [invoice] = await createTestInvoice(adminClient, dummyOrganization, {
                paymentStatusChangeWebhookUrl: callbackUrl,
            })
            ...
            // 3. Store the secret - decrypt it first since EncryptedText fields are returned encrypted
            const encryptionManager = new EncryptionManager()
            storedSecrets[invoice.id] = encryptionManager.decrypt(invoice.paymentStatusChangeWebhookSecret)

            // 4. Create a payment linked to this invoice
            const [payment] = await createTestPayment(adminClient, dummyOrganization, null, null, {
                invoice,
            ...
            // 5. Update payment status to trigger webhook (worker will process this)
            const { updateTestPayment } = require('@condo/domains/acquiring/utils/testSchema')
            const [updatedPayment] = await updateTestPayment(adminClient, payment.id, {
                status: PAYMENT_DONE_STATUS,
            })
            expect(updatedPayment.status).toBe(PAYMENT_DONE_STATUS)

            // 6. Wait for WebhookPayload record to be created
            await waitFor(async () => {
                const payloads = await WebhookPayload.getAll(adminClient, {
                    itemId: payment.id,
                    modelName: 'Payment',
                })
            ...
            // 7. Wait for webhook to be received by Express server
            await waitFor(async () => {
                expect(receivedWebhooks[payment.id]).toBeDefined()
            }, { timeout: 10000 })

            // 8. Verify the received webhook
            const receivedWebhook = receivedWebhooks[payment.id]

            expect(receivedWebhook.signatureValid).toBe(true)
            expect(receivedWebhook.payload.__typename).toBe('Payment')
            expect(receivedWebhook.payload.id).toBe(payment.id)
            expect(receivedWebhook.payload.status).toBe(PAYMENT_DONE_STATUS)
            expect(receivedWebhook.payload.invoice.__typename).toBe('Invoice')
            expect(receivedWebhook.payload.invoice.id).toBe(invoice.id)

            // Clean up
            delete receivedWebhooks[payment.id]
            delete storedSecrets[invoice.id]
        })
    })
})
