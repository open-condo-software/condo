/**
 * Generated by `createschema marketplace.Invoice 'number:Integer; property:Relationship:Property:PROTECT; unitType:Text; unitName:Text; accountNumber:Text; toPay:Decimal; items:Json; contact?:Relationship:Contact:SET_NULL; client?:Relationship:User:SET_NULL; clientName?:Text; clientPhone?:Text; clientEmail?:Text; statusChangeCallbackSecret:Text'`
 */
...
const { MarketItem, InvoiceWebhookDelivery } = require('@condo/domains/marketplace/utils/serverSchema')
const {
...
const Invoice = new GQLListSchema('Invoice', {
    ...
    fields: {
        ...
        statusChangeCallbackUrl: {
            schemaDoc: 'URL to call when invoice status changes. When set, the system will send HTTP POST requests to this URL with invoice status change details. Delivery is guaranteed with retries for up to INVOICE_WEBHOOK_DELIVERY_TTL_DAYS days.',
            type: 'Url',
            isRequired: false,
        },

        statusChangeCallbackSecret: {
            schemaDoc: 'Secret key used to sign webhook payloads for this invoice. Auto-generated when statusChangeCallbackUrl is set. The receiver should use this secret to verify the X-Condo-Signature header.',
            type: 'Text',
            isRequired: false,
            access: {
                read: true,
    ...
    hooks: {
        ...
        resolveInput: async ({ context, operation, resolvedData, existingItem }) => {
            ...
            // Auto-generate webhook secret when callback URL is set for the first time
            ...
            if (newCallbackUrl && !existingSecret) {
                ...
                resolvedData['statusChangeCallbackSecret'] = randomBytes(32).toString('hex')
        ...
        afterChange: async ({ context, originalInput, updatedItem, existingItem, operation }) => {
            ...
            // Trigger webhook delivery on status change
            const previousStatus = existingItem?.status || null
            const newStatus = updatedItem.status
            const callbackUrl = updatedItem.statusChangeCallbackUrl
            ...
            if (callbackUrl && statusChanged) {
                ...
                try {
                    ...
                    // Log error but don't fail the invoice operation
                    logger.error({
                        msg: 'Failed to create webhook delivery',
                        data: { invoiceId: updatedItem.id },
                        err,
                    })
                }
            }
