...
async function canReadBankTransactionsSyncTasks ({ authentication: { item: user } }) {
...
async function canManageBankTransactionsSyncTasks ({ authentication: { item: user }, originalInput, operation, itemId }) {
    if (!user) return throwAuthenticationError()
    if (user.deletedAt) return false
    ...
    if (operation === 'update') {
        const syncTask = await getById('BankTransactionsSyncTask', itemId)
...
*/
module.exports = {
    canReadBankTransactionsSyncTasks,
    canManageBankTransactionsSyncTasks,
}
