...
describe('validateTin', () => {
    VALID_RU_TINS.forEach(tin => {
        test(`for valid 10 or 12 char RU TIN (${tin})`, () => {
            const { result } = validateTin(tin, COUNTRY_CODE_RU)
            expect(result).toBe(true)
        })

        SPACES.forEach(spaceSymbol => {
            test(`for valid 10 or 12 char RU TIN (${tin}) with spaces symbol (${SPACE_SYMBOL_LABLES[spaceSymbol] || spaceSymbol})`, () => {
    ...
    test('wrong length number as RU TIN', () => {
    ...
    test('for invalid control sum as RU TIN with 12 characters', () => {
        const { result, errors } = validateTin(INVALID_CONTROL_SUM_TIN_12, COUNTRY_CODE_RU)
        expect(result).toBe(false)
        expect(errors[0]).toBe('Control sum is not valid for tin')
    })
    test('for create valid RU TIN with 10 characters', () => {
        const tin = createValidRuTin10()
        const { result } = validateTin(tin, COUNTRY_CODE_RU)
        expect(result).toBe(true)
    })
    test('for create valid RU TIN with 12 characters', () => {
