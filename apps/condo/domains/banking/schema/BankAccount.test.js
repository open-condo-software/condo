...
describe('BankAccount', () => {
    ...
    describe('fields', () => {
        ...
        describe('name', () => {
            test('name field populated from organization if not provided', async () => {
                const [organization] = await createTestOrganization(adminClient)
                const [createdObj] = await createTestBankAccount(adminClient, organization)
                expect(createdObj.name).toEqual(organization.name)
            })

            test('name field populated from organization if it\'s removed on update', async () => {
                const [organization] = await createTestOrganization(adminClient)
                const bankAccountName = faker.random.alpha(10)
                const [createdObj] = await createTestBankAccount(adminClient, organization, { name: bankAccountName })
                expect(createdObj.name).toEqual(bankAccountName)

                const [updatedObj] = await updateTestBankAccount(adminClient, createdObj.id, { name: null })
                expect(updatedObj.name).toEqual(organization.name)
            })
        })
