...
const BankAccount = new GQLListSchema('BankAccount', {
    ...
    fields: {
        ...
        name: {
            ...
            isRequired: false,
            hooks: {
                resolveInput: async ({ operation, fieldPath, resolvedData, existingItem }) => {
                    let organizationId = get(resolvedData, 'organization')
                    if (operation === 'update') {
                        organizationId = get(existingItem, 'organization')
                    }
                    if (operation === 'create' || operation === 'update') {
                        const resolvedName = get(resolvedData, fieldPath)
                        if (!isNil(resolvedName)) {
