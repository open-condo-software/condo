/**
 * Generated by `createschema document.DocumentCategory 'name:Text; order:Integer;'`
 * In most cases you should not change it by hands
 * Please, don't remove `AUTOGENERATE MARKER`s
 */

const { generateGqlQueries } = require('@open-condo/codegen/generate.gql')


const COMMON_FIELDS = 'id dv sender { dv fingerprint } v deletedAt newId createdBy { id name } updatedBy { id name } createdAt updatedAt'

const DOCUMENT_CATEGORY_FIELDS = `{ name ${COMMON_FIELDS} }`
const DocumentCategory = generateGqlQueries('DocumentCategory', DOCUMENT_CATEGORY_FIELDS)

const DOCUMENT_FIELDS = `{ organization { id } property { id } category { id name } name file { id originalFilename publicUrl mimetype } canReadByResident anonymousCanRead publicAccessToken publicFileUrl ${COMMON_FIELDS} }`
const Document = generateGqlQueries('Document', DOCUMENT_FIELDS)

/* AUTOGENERATE MARKER <CONST> */

module.exports = {
    DocumentCategory,
    Document,
/* AUTOGENERATE MARKER <EXPORTS> */
}
