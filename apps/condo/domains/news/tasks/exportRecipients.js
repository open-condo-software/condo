...
const { PropertyAddressAndMap } = require('@condo/domains/property/utils/serverSchema')
        ...
        if (isAllOrganization) {
            ...
            for (let newsItemScope of compactedNewsItemScopes) {
                if (get(newsItemScope, 'property.id')) {
                    const property = await PropertyAddressAndMap.getOne(context, {
