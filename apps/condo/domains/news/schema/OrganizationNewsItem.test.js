    ...
    describe('CRUD tests', () => {
        describe('create', () => {
            ...
            test('stuff with permission can', async () => {
                ...
                const [role] = await createTestOrganizationEmployeeRole(adminClient, o10n, { canManageOrganizationNews: true })
            ...
            test('staff without permission can\'t', async () => {
                const client = await makeClientWithNewRegisteredAndLoggedInUser()
                const [o10n] = await createTestOrganization(adminClient)
                const [role] = await createTestOrganizationEmployeeRole(adminClient, o10n, { canManageOrganizationNews: false })
                await createTestOrganizationEmployee(adminClient, o10n, client.user, role)
        ...
        describe('read', () => {
            ...
            test('stuff can', async () => {
                const [o10n] = await createTestOrganization(adminClient)
                const [objCreated] = await createTestOrganizationNewsItem(adminClient, o10n)
                const [roleWithAccess] = await createTestOrganizationEmployeeRole(adminClient, o10n, { canManageOrganizationNews: true })
                const [roleWithoutAccess] = await createTestOrganizationEmployeeRole(adminClient, o10n, { canManageOrganizationNews: false })
