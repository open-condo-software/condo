...
describe('GetNewsItemsRecipientsCountersService', () => {
    ...
    test('The data for counters is calculated correctly', async () => {
        ...
        /**
         * Duplicate unit scopes must be counted once
         */
        const payloadDuplicateUnits = {
            organization: pick(dummyO10n, 'id'),
            newsItemScopes: [
                { property: { id: property1.id }, unitType: FLAT_UNIT_TYPE, unitName: '1' },
                { property: { id: property1.id }, unitType: FLAT_UNIT_TYPE, unitName: '1' },
        ...
        const [dataDuplicateUnits] = await getNewsItemsRecipientsCountersByTestClient(staffClientYes, payloadDuplicateUnits)
        expect(dataDuplicateUnits).toEqual({ propertiesCount: 1, unitsCount: 1, receiversCount: 1 })

        /**
         * Type-wide unitType without unitName: all map units of that type + residents of that type (including off-map)
         */
        const payloadTypeWideFlats = {
            organization: pick(dummyO10n, 'id'),
            newsItemScopes: [
                { property: { id: property1.id }, unitType: FLAT_UNIT_TYPE, unitName: null },
            ],
        }
        const [dataTypeWideFlats] = await getNewsItemsRecipientsCountersByTestClient(staffClientYes, payloadTypeWideFlats)
        expect(dataTypeWideFlats).toEqual({ propertiesCount: 1, unitsCount: 36, receiversCount: 5 })

        const payloadTypeWideParking = {
            organization: pick(dummyO10n, 'id'),
            newsItemScopes: [
                { property: { id: property1.id }, unitType: PARKING_UNIT_TYPE, unitName: null },
            ],
        }
        const [dataTypeWideParking] = await getNewsItemsRecipientsCountersByTestClient(staffClientYes, payloadTypeWideParking)
        expect(dataTypeWideParking).toEqual({ propertiesCount: 1, unitsCount: 0, receiversCount: 3 })
    })
    ...
    test('The data for counters without propertyMap is calculated correctly', async () => {
        ...
        /**
         * Type-wide unitType without a property map still counts residents of that type
         */
        const payloadTypeWideNoMap = {
            organization: pick(dummyO10n, 'id'),
            newsItemScopes: [
                { property: { id: property1.id }, unitType: FLAT_UNIT_TYPE, unitName: null },
            ],
        }
        const [dataTypeWideNoMap] = await getNewsItemsRecipientsCountersByTestClient(staffClientYes, payloadTypeWideNoMap)
        expect(dataTypeWideNoMap).toEqual({ propertiesCount: 1, unitsCount: 0, receiversCount: 4 })
    })

    test('Unit-level counters skip deleted and missing properties', async () => {
        const [property] = await createTestProperty(adminClient, dummyO10n, { map: propertyMap1x9x4 })
        const [deletedProperty] = await createTestProperty(adminClient, dummyO10n, { map: propertyMap1x9x4 })

        const [user] = await createTestUser(adminClient)
        await createTestResident(adminClient, user, property, { unitType: FLAT_UNIT_TYPE, unitName: '1' })
        await createTestResident(adminClient, user, deletedProperty, { unitType: FLAT_UNIT_TYPE, unitName: '1' })

        await updateTestProperty(adminClient, deletedProperty.id, { deletedAt: 'true' })

        const missingPropertyId = faker.datatype.uuid()

        const [data] = await getNewsItemsRecipientsCountersByTestClient(staffClientYes, {
            organization: pick(dummyO10n, 'id'),
            newsItemScopes: [
                { property: { id: property.id }, unitType: FLAT_UNIT_TYPE, unitName: '1' },
                { property: { id: deletedProperty.id }, unitType: FLAT_UNIT_TYPE, unitName: '1' },
                { property: { id: missingPropertyId }, unitType: FLAT_UNIT_TYPE, unitName: '1' },
            ],
        })
        expect(data).toEqual({ propertiesCount: 1, unitsCount: 1, receiversCount: 1 })
    })
