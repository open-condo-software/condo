...
const NEWS_ITEM_FIELDS = `{ organization { id } number title body type validBefore sendAt sentAt isPublished publishedAt source { id type name } user { id name } ${COMMON_FIELDS} compactScopes { count firstOnes { id unitType unitName property { address addressMeta { ${ADDRESS_META_SUBFIELDS_TABLE_LIST} } } } } }`
...
const NEWS_ITEM_SOURCE_FIELDS = `{ type name isDefault ${COMMON_FIELDS} }`
const NewsItemSource = generateGqlQueries('NewsItemSource', NEWS_ITEM_SOURCE_FIELDS)
