...
const { get, omit, uniq, isEmpty } = require('lodash')
...
const { checkBillingAccountNumberForOrganization } = require('@condo/domains/billing/utils/serverSchema/checkBillingAccountNumberForOrganization')
...
const RegisterServiceConsumerService = new GQLCustomSchema('RegisterServiceConsumerService', {
    ...
    mutations: [
        ...
        {
            doc: {
                summary: 'Creates service consumer with default data, and automatically populates the optional data fields, such as `billingContext',
                description: 'To be successfully created accountNumber should at least have billingAccount with same number or Meter with same accountNumber',
                errors: ERRORS,
            },
            access: access.canRegisterServiceConsumer,
            schema: 'registerServiceConsumer(data: RegisterServiceConsumerInput!): ServiceConsumer',
            resolver: async (parent, args, context = {}) => {
                const { data: { dv, sender, residentId, accountNumber: accountNumberRaw, organizationId, extra } } = args
                ...
                const { isBillingAccountFound, contexts } = await checkBillingAccountNumberForOrganization(organizationId, accountNumber)
                const meters = await Meter.getAll(context, {
                    accountNumber: accountNumber,
                    organization: { id: organizationId },
                    deletedAt: null,
                })
                if (!isBillingAccountFound && !meters.length) {
                    throw new GQLError(ERRORS.BILLING_ACCOUNT_NOT_FOUND, context)
                }
                let [existingServiceConsumer] = await ServiceConsumer.getAll(context, {
                    resident: { id: residentId },
                    accountNumber: accountNumber,
                    organization: { id: organizationId },
                    deletedAt: null,
                })
                // Will be removed after removing this fields from the serviceConsumer model
                if (existingServiceConsumer && isBillingAccountFound) {
                    const updateInput = {}
                    if (!existingServiceConsumer.billingIntegrationContext) {
                        updateInput.billingIntegrationContext = { connect: { id: contexts.billing.id } }
                    }
                    if (!existingServiceConsumer.acquiringIntegrationContext) {
                        updateInput.acquiringIntegrationContext = { connect: { id: contexts.acquiring.id } }
                    }
                ...
                if (!existingServiceConsumer) {
                    existingServiceConsumer = await ServiceConsumer.create(context, {
                        dv, sender,
                        // Will be removed after removing this fields from the serviceConsumer model
                        ...isBillingAccountFound ? {
                            billingIntegrationContext: { connect: { id: contexts.billing.id } },
                            acquiringIntegrationContext: { connect: { id: contexts.acquiring.id } },
                        }  : {},
                ...
                // TODO(zuch): DOMA-9033 need to fix before database replication
                return await getById('ServiceConsumer', existingServiceConsumer.id)
            },
        },
    ],
})
