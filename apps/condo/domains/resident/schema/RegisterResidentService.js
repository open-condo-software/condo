    ...
    mutations: [
        {
            ...
            resolver: async (parent, args, context) => {
                const { data: { dv, sender, address, addressMeta, rentalUnit: rentalUnitId } } = args
                ...
                const existingResidents = await ResidentAPI.getAll(context, {
                    user: { id: context.authedItem.id },
                    deletedAt: null,
                }, 'id address addressMeta unitName')
                const requestedAddress = getAddressUpToBuildingFrom(addressMeta) || addressItem.address
                const hasDuplicateResident = existingResidents.some(resident => {
                    const residentAddress = getAddressUpToBuildingFrom(get(resident, 'addressMeta')) || resident.address

                    return normalizeString(residentAddress) === normalizeString(requestedAddress)
                        && normalizeString(resident.unitName) === normalizeString(rentalUnit.name)
                })
