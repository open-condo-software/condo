...
describe('RegisterResidentService', () => {
    ...
    describe('DiscoverServiceConsumer hook', () => {
        it('should create ServiceConsumer for registered Resident with one BillingAccount', async () => {
            const adminClient = await makeLoggedInAdminClient()
            ...
            const createdServiceConsumers = await ServiceConsumer.getAll(adminClient, {
        ...
        it('should create ServiceConsumer for registered Resident with multiple BillingAccounts', async () => {
            const adminClient = await makeLoggedInAdminClient()
            const { context } = await makeContextWithOrganizationAndIntegrationAsAdmin(
                {}, {}, { status: 'Finished' }
            )
            ...
            expect(createdServiceConsumers.map((serviceConsumer) => serviceConsumer.accountNumber).toString()).toContain(billingAccount2.number)
            expect(createdServiceConsumers[0].resident.id).toEqual(resident.id)

        })

        it('should not create ServiceConsumer for registered Resident without BillingAccount', async () => {
            const adminClient = await makeLoggedInAdminClient()
            const user = await makeClientWithResidentUser()
            const unitName = faker.random.alphaNumeric(8)
            const { address, addressMeta } = buildFakeAddressAndMeta(false)
