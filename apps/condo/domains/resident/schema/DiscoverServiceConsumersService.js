 ...
 */
const { set, get, filter, flatMap, omit } = require('lodash')
    ...
    mutations: [
        {
            ...
            resolver: async (parent, args, context) => {
                ...
                await loadListByChunks({
                    ...
                    where: {
                        deletedAt: null,
                        organization: { id_in: billingAccountItemsData.map(({ organizationId }) => organizationId) },
                ...
                for (const item of billingAccountItemsData) {
                    ...
                    for (const receipt of receipts) {
                        const theKey = `${item.address}_${item.unitType}_${item.unitName}_${receipt.category.id}_${receipt.period}`
                        /** @type {Set} */
