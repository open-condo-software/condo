...
const {
    ...
    updateTestOrganizationEmployee, createTestOrganizationEmployee,
...
describe('Ticket', () => {
    describe('Crud', () => {
        ...
        test('user: set ticket assignee and executor', async () => {
            const client = await makeClientWithProperty()
            const [objCreated] = await createTestTicket(client, client.organization, client.property)

            const payload = {
                details: 'Ticket created without assignee and executor, that would be connected a bit later',
        ...
        test('user: create ticket with assignee and executor', async () => {
            const client = await makeClientWithProperty()
            const payload = {
                details: 'Ticket created with connected assignee and executor directly',
                assignee: { connect: { id: client.user.id } },
                executor: { connect: { id: client.user.id } },
            }
            const [objCreated] = await createTestTicket(client, client.organization, client.property, payload)

            expect(objCreated.assignee).toEqual(expect.objectContaining({ id: client.user.id }))
            expect(objCreated.executor).toEqual(expect.objectContaining({ id: client.user.id }))
        })

        test('user: create ticket with assignee and executor and change assignee', async () => {
            const client = await makeClientWithProperty()
            const admin = await makeLoggedInAdminClient()
            const [organization] = await createTestOrganization(admin)
            ...
            const payload1 = { assignee: { disconnectAll: true,  connect: { id: managerUserClient.user.id } } }
            const [objUpdated] = await updateTestTicket(admin, objCreated.id, payload1)

            expect(objUpdated.assignee).toEqual(expect.objectContaining({ id: managerUserClient.user.id }))
        })

        test('user: create ticket with connected assignee having registered pushToken ', async () => {
            const client = await makeClientWithProperty()
            const tokenData = {
                pushToken: FIREBASE_PUSH_TOKEN_TEST || PUSH_FAKE_TOKEN_SUCCESS,
                pushTransport: PUSH_TRANSPORT_FIREBASE,
            }

            const tokens = await find('Device', { ...tokenData, deletedAt: null })

            // We reuse FIREBASE_PUSH_TOKEN_TEST || PUSH_FAKE_TOKEN_SUCCESS,
            // but because of Device schema having unique constraint on pushToken field,
            // we should clean up all previous usages of same value for pushToken,
            // before we can use it again
            if (!isEmpty(tokens)) {
                for (const token of tokens) {
                    const payload = {
                        deviceId: token.deviceId,
                        pushTransport: token.pushTransport,
                        pushToken: null,
                    }

                    await syncDeviceByTestClient(client, payload)
                }
            }
            ...
            const [objCreated] = await createTestTicket(client, client.organization, client.property, payload2)
