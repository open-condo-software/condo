...
const { Relationship, Virtual, Text } = require('@keystonejs/fields')
...
const TicketChange = new GQLListSchema('TicketChange', {
    ...
    fields: {
        ...
        changedByName: {
            schemaDoc: 'Freeze name of user who made ticket change (read-only)',
            type: Text,
            access: {
                read: true,
                update: false,
    ...
    hooks: {
        resolveInput: ({ context, resolvedData }) => {
            resolvedData.changedByName = get(context, ['authedItem', 'name'])
