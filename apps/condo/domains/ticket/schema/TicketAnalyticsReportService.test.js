...
describe('TicketAnalyticsReportService', () => {
    describe('User', () => {
        it('can read TicketAnalyticsReportService grouped counts [day, status]', async () => {
            ...
            } = await client.query(TICKET_ANALYTICS_REPORT_QUERY, {
                dv: 1,
        ...
        it('can read TicketAnalyticsReportService with null categoryClassifier', async () => {
            ...
            expect(groups.filter((group) => group.count === 1)[0].categoryClassifier).toStrictEqual(
                NULL_REPLACES['categoryClassifier'],
            )
        })
        ...
        it('can read TicketAnalyticsReportService with null executor', async () => {
            ...
            expect(groups.filter((group) => group.count === 1)[0].executor).toStrictEqual(NULL_REPLACES['executor'])
        })
        ...
        it('can not read TicketAnalyticsReportService from another organization', async () => {
            ...
            const {
                data: {
                    result: { groups: emptyGroups },
                },
            } = await wrongClient.query(TICKET_ANALYTICS_REPORT_QUERY, {
                dv: 1,
                sender: { dv: 1, fingerprint: 'tests' },
                data: {
                    where: {
        ...
        it('can read exportTicketAnalyticsToExcel with selected organization', async () => {
            if (isObsConfigured()) {
                ...
                } = await client.query(EXPORT_TICKET_ANALYTICS_TO_EXCEL, {
                    dv: 1,
                    sender: { dv: 1, fingerprint: 'tests' },
                    data: {
                        where: {
        ...
        it('can not read exportTicketAnalyticsToExcel from another organization', async () => {
            ...
            const {
                data: { result },
                errors,
            } = await restrictedClient.query(EXPORT_TICKET_ANALYTICS_TO_EXCEL, {
                dv: 1,
                sender: { dv: 1, fingerprint: 'tests' },
                data: {
                    where: {
    ...
    describe('Anonymous', () => {
        ...
        it('can not get ticket analytics export', async () => {
            ...
            const {
                data: { result },
                errors,
            } = await anonymousClient.query(EXPORT_TICKET_ANALYTICS_TO_EXCEL, {
                data: {
                    where: {
                        organization: { id: client.organization.id },
                        AND: [{ createdAt_gte: dateStart.toISOString() }, { createdAt_lte: dateEnd.toISOString() }],
