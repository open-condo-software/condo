...
describe('TicketFile', () => {
    test('user: create TicketFile', async () => {
        ...
        const [obj, attrs] = await createTestTicketFile(client)  // TODO(codegen): write 'user: create TicketFile' test
    ...
    test('anonymous: create TicketFile', async () => {
        ...
        try {
            await createTestTicketFile(client)  // TODO(codegen): check the 'anonymous: create TicketFile' test!
        } catch (e) {
    ...
    test('user: read TicketFile', async () => {
        ...
        const [obj, attrs] = await createTestTicketFile(admin)  // TODO(codegen): check create function!

        const client = await makeClient()  // TODO(codegen): use truly useful client!
        const objs = await TicketFile.getAll(client, {}, { sortBy: ['updatedAt_DESC'] })

        // TODO(codegen): check 'user: read TicketFile' test!
    ...
    test('user: update TicketFile', async () => {
        const admin = await makeLoggedInAdminClient()
        const [objCreated] = await createTestTicketFile(admin)  // TODO(codegen): check create function!

        const client = await makeClient()  // TODO(codegen): use truly useful client!
        const payload = {}  // TODO(codegen): change the 'user: update TicketFile' payload
        const [objUpdated, attrs] = await updateTestTicketFile(client, objCreated.id, payload)

        // TODO(codegen): white checks for 'user: update TicketFile' test
    ...
    test('anonymous: update TicketFile', async () => {
        ...
        const client = await makeClient()
        const payload = {}  // TODO(codegen): change the 'anonymous: update TicketFile' payload
        try {
            await updateTestTicketFile(client, objCreated.id, payload)
        } catch (e) {
            expect(e.errors[0]).toMatchObject({
                'message': 'You do not have access to this resource',
                'name': 'AccessDeniedError',
    ...
    test('user: delete TicketFile', async () => {
        ...
        try {
            ...
            await TicketFile.delete(client, objCreated.id)
        } catch (e) {
            expect(e.errors[0]).toMatchObject({
                'message': 'You do not have access to this resource',
                'name': 'AccessDeniedError',
