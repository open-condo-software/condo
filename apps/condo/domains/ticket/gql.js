...
const TICKET_FIELDS = `{ canReadByResident completedAt lastCommentAt isResidentTicket reviewValue reviewComment deadline deferredUntil organization { id name country } property { ${TICKET_PROPERTY_FIELDS} } propertyAddress propertyAddressMeta { ${ADDRESS_META_SUBFIELDS_QUERY_LIST} } unitType unitName sectionName sectionType floorName status { id name type organization { id } colors { primary secondary additional } } statusReopenedCounter statusUpdatedAt statusReason number client { id name } clientName clientEmail clientPhone contact { id name phone unitName unitType } operator { id name } assignee { id name } executor { id name } details related { id details } isEmergency isPaid isWarranty meta source { id name type } sourceMeta categoryClassifier { id } placeClassifier { id } problemClassifier { id } ${TICKET_CLASSIFIER_ATTRIBUTES_FIELDS} ${COMMON_FIELDS} }`
...
const TICKET_ORGANIZATION_SETTING_FIELDS = '{ id organization { id } defaultDeadline paidDeadline emergencyDeadline warrantyDeadline }'
const TicketOrganizationSetting = generateGqlQueries('TicketOrganizationSettings', TICKET_ORGANIZATION_SETTING_FIELDS)
