...
function MappingForm({ columns, cols, onChangeMapping, onFinish }) {
    ...
    return (
        <Form layout={'horizontal'} size={'small'} form={form} onValuesChange={handleChanges} onFinish={onFinish}>
            ...
            {columns.map((column, index) => {
                return (
                    <Form.Item key={index} label={[ColumnMessage, ' "', column.title, '"']} name={column.dataIndex}>
...
function ExcelExporterButton({ columns, setExportedData }) {
    ...
    const ClickOrDragImportFileTextMessage = intl.formatMessage({
        id: 'containers.FormTableExcelImport.ClickOrDragImportFileText',
    })
    const ClickOrDragImportFileHintMessage = intl.formatMessage({
        id: 'containers.FormTableExcelImport.ClickOrDragImportFileHint',
    })
    ...
    const formattedFiles = TABLE_UPLOAD_ACCEPT_FILES.map(function (x) {
        return '.' + x
    }).join(',')
    ...
    return (
        <>
            <CreateFormListItemButton
                onClick={() => setStep(1)}
                label={ImportFromFileButtonLabel}
                style={{ marginBottom: '16px', width: '100%', display: step === -1 ? 'block' : 'none' }}
            />
            {step > 0 ? <Progress percent={step * 33.33} strokeColor="#52c41a" showInfo={false} /> : null}
            {StepHelpText[step] ? <Typography.Paragraph>{StepHelpText[step]}</Typography.Paragraph> : null}
