...
function DefaultCellInner({ children, record, rowIndex, column }) {
    ...
    if (editing) {
        return (
            <Form.Item style={{ margin: 0 }} name={column.dataIndex}>
                <Input ref={inputRef} onPressEnter={handleEdited} onBlur={handleEdited} />
            </Form.Item>
        )
    } else {
        return (
            <div onClick={toggleEdit} key={record[column.dataIndex]}>
                {children}
            </div>
        )
    }
...
function DefaultRowInner({ children, record, rowIndex }) {
    const form = useContext(RowFormContext)
    return (
        <Form name={`row${rowIndex}`} form={form} component={false}>
            {children}
        </Form>
    )
}
...
function FormTable({
    ...
    dataSource,
    pagination,
    onChangeFilterPaginationSort,
    rowContextInitialState,
    ...
    return (
        <TableContext.Provider value={{ ...tableContextInitialState, rowContextInitialState, RowInner, CellInner }}>
            <Table
                rowKey={'id'}
                tableLayout={'fixed'}
                size="small"
