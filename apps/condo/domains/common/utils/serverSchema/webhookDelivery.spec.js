...
const { WEBHOOK_TIMEOUT_MS, WEBHOOK_RETRY_INTERVALS } = require('@condo/domains/common/constants/webhook')
const {
    generateSignature,
...
describe('webhookDelivery utilities', () => {
    ...
    describe('calculateNextRetryAt', () => {
        ...
        test('should return 1 minute delay for attempt 1', () => {
            ...
            const expectedDelay = WEBHOOK_RETRY_INTERVALS[1] // 60 seconds
        ...
        test('should use last interval for attempts beyond array length', () => {
            ...
            const lastInterval = WEBHOOK_RETRY_INTERVALS[WEBHOOK_RETRY_INTERVALS.length - 1]
    ...
    describe('tryDeliverWebhook', () => {
        ...
        test('should return failure if url is missing', async () => {
            ...
            const result = await tryDeliverWebhook(deliveryWithoutUrl)

            expect(result.success).toBe(false)
            expect(result.error).toContain('Missing required delivery fields')
        })

        test('should return failure if payload is missing', async () => {
            const deliveryWithoutPayload = { ...mockDelivery, payload: null }

            const result = await tryDeliverWebhook(deliveryWithoutPayload)
        ...
        test('should return failure if secret is missing', async () => {
            ...
            const result = await tryDeliverWebhook(deliveryWithoutSecret)
        ...
        test('should handle payload as string', async () => {
            ...
            const result = await tryDeliverWebhook(deliveryWithStringPayload)
