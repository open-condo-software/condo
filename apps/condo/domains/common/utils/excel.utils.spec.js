...
test('toExData() with undefined', () => {
    expect(toExData([[1, 2], [3, 4], [5]])).toEqual([
        [{ value: 1 }, { value: 2 }],
        [{ value: 3 }, { value: 4 }],
        [{ value: 5 }, { value: undefined }],
    ])
})
...
test('reValidateExData(email)', () => {
    const d = toExData([
        ['email', 'points'],
        ['teSt@example.com', 4],
        ['Победа <pobeda@info.pobeda.aero>', 6],
    ])
...
test('VALIDATORS(name)', () => {
    ...
    expect(validate('name', 'Мазп---а靷ễша-֊־᐀᠆‐-―⸗⸚〜〰 ゠︱ Ё︲﹘ ﹣－енов Живикин - Клубникин').cleanedValue).toEqual(
        'Мазп-а靷ễша-Ё-енов Живикин-Клубникин',
    )
