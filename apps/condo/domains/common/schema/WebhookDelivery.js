...
const access = require('@condo/domains/common/access/WebhookDelivery')
const {
...
const WebhookDelivery = new GQLListSchema('WebhookDelivery', {
    schemaDoc: 'Stores webhook delivery attempts. Contains ready-to-send payload, URL, and secret. Tracks delivery status, retries, and responses from external servers.',
    fields: {
        ...
        status: {
            ...
            defaultValue: WEBHOOK_DELIVERY_STATUS_PENDING,
            isRequired: true,
        },
        ...
        lastResponseBody: {
            schemaDoc: `Response body from the last delivery attempt (truncated to ${WEBHOOK_MAX_RESPONSE_LENGTH} chars)`,
            type: 'Text',
            isRequired: false,
        },
            ...
            hooks: {
                resolveInput: ({ resolvedData, operation, fieldPath }) => {
                    if (operation === 'create' && !resolvedData[fieldPath]) {
                        return dayjs().add(WEBHOOK_DELIVERY_TTL_DAYS, 'day').toISOString()
                    }
    ...
    access: {
        read: access.canReadWebhookDeliveries,
        create: access.canManageWebhookDeliveries,
        update: access.canManageWebhookDeliveries,
        delete: false,
        auth: true,
    },
})
