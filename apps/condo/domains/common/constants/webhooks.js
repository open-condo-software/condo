const WEBHOOK_EVENT_PAYMENT_STATUS_UPDATED = 'payment.status.updated'
const WEBHOOK_EVENT_SUBSCRIPTION_ACTIVATED = 'subscription.activated'
const WEBHOOK_EVENT_SUBSCRIPTION_INVOICE_REQUESTED = 'subscription.invoice.requested'
const WEBHOOK_EVENT_PAYMENTS_FILE_STATUS_UPDATED = 'paymentsFile.status.updated'

const WEBHOOK_EVENTS = [
    WEBHOOK_EVENT_PAYMENT_STATUS_UPDATED,
    WEBHOOK_EVENT_SUBSCRIPTION_ACTIVATED,
    WEBHOOK_EVENT_SUBSCRIPTION_INVOICE_REQUESTED,
    WEBHOOK_EVENT_PAYMENTS_FILE_STATUS_UPDATED,
]

module.exports = {
    WEBHOOK_EVENT_PAYMENT_STATUS_UPDATED,
    WEBHOOK_EVENT_SUBSCRIPTION_ACTIVATED,
    WEBHOOK_EVENT_SUBSCRIPTION_INVOICE_REQUESTED,
    WEBHOOK_EVENT_PAYMENTS_FILE_STATUS_UPDATED,
    WEBHOOK_EVENTS,
}
