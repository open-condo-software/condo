...
const { ERRORS } = require('./AuthenticateOrRegisterUserWithConfirmTokenService')
...
describe('AuthenticateOrRegisterUserWithConfirmTokenService', () => {
    ...
    describe('Accesses', () => {
        describe('User', () => {
            test('can execute', async () => {
                ...
                const [result] = await authenticateOrRegisterUserWithConfirmTokenByTestClient(userClient, {
        ...
        describe('Anonymous (anyone)', () => {
            test('can execute', async () => {
                const [confirmPhoneAction] = await createTestConfirmPhoneAction(adminClient, { isPhoneVerified: true })
                const [result] = await authenticateOrRegisterUserWithConfirmTokenByTestClient(anonymousClient, {
                    confirmToken: confirmPhoneAction.token,
                    userType: RESIDENT,
                })
                expect(result.token).not.toHaveLength(0)
    ...
    describe('Base logic', () => {
        describe('For user type = "resident"', () => {
            ...
            test('should sign in resident with confirmed phone token if a resident with such a phone is already registered', async () => {
                ...
                const [result2] = await authenticateOrRegisterUserWithConfirmTokenByTestClient(anonymousClient2, {
                    confirmToken: confirmPhoneAction2.token,
                    userType: RESIDENT,
                })
        ...
        describe('For user type = "staff"', () => {
            test('should register and sign in staff with confirmed phone token if a staff with such a phone is not registered yet', async () => {
                const [confirmPhoneAction] = await createTestConfirmPhoneAction(adminClient, { isPhoneVerified: true })
                const [result, attrs] = await authenticateOrRegisterUserWithConfirmTokenByTestClient(anonymousClient, {
                    confirmToken: confirmPhoneAction.token,
                    userType: STAFF,
                    userData: {
                        name: faker.name.fullName(),
        ...
        test('should be set empty user data from the specified user data if the user is already registered', async () => {
            ...
            const [result2, attrs2] = await authenticateOrRegisterUserWithConfirmTokenByTestClient(anonymousClient, {
                confirmToken: confirmPhoneAction2.token,
                userType: RESIDENT,
                userData: {
                    name: faker.name.fullName(),
        ...
        test('should save current non-empty user data if the user is already registered and new user data is passed in the payload', async () => {
            ...
            const [result2] = await authenticateOrRegisterUserWithConfirmTokenByTestClient(anonymousClient, {
                confirmToken: confirmPhoneAction2.token,
                userType: RESIDENT,
                userData: {
                    name: faker.name.fullName(),
