...
describe('Validations', () => {
    describe('Password', () => {
        test('Valid password should be save', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()

            const [updatedUser] = await updateTestUser(client, client.user.id, {
                password: faker.internet.password(12),
            })
            expect(client.user.id).toEqual(updatedUser.id)
        })

        it('must be in string format', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()

            await expectToThrowGraphQLRequestError(async () => {
                await updateTestUser(client, client.user.id, {
                    password: faker.datatype.number(),
                })
            }, 'String cannot represent a non string value')
        })

        it('must not start or end with a space', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()
        ...
        it('must be of the appropriate length', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()
            ...
            await catchErrorFrom(async () => {
                await updateTestUser(client, client.user.id, {
                    password: faker.internet.password(MAX_PASSWORD_LENGTH + 1),
                })
            }, ({ errors }) => {
                expect(errors).toHaveLength(1)
                expect(errors[0]).toEqual(expect.objectContaining({
        ...
        it('must consist of different characters', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()

            await catchErrorFrom(async () => {
                await updateTestUser(client, client.user.id, {
                    password: faker.internet.password(12, false, /a+/),
                })
            }, ({ errors }) => {
                expect(errors).toHaveLength(1)
                expect(errors[0]).toEqual(expect.objectContaining({
                    message: ERRORS.PASSWORD_CONSISTS_OF_IDENTICAL_CHARACTERS.message,
                }))
            })
        })

        it('must not contain email', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()

            await catchErrorFrom(async () => {
                await updateTestUser(client, client.user.id, {
                    password: client.userAttrs.email + faker.internet.password(12),
                })
            }, ({ errors }) => {
                expect(errors).toHaveLength(1)
                expect(errors[0]).toEqual(expect.objectContaining({
        ...
        it('must not contain phone', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()

            await catchErrorFrom(async () => {
                await updateTestUser(client, client.user.id, {
                    password: client.userAttrs.phone + faker.internet.password(12),
                })
            }, ({ errors }) => {
                expect(errors).toHaveLength(1)
                expect(errors[0]).toEqual(expect.objectContaining({
        ...
        it('must not contain name', async () => {
            const client = await makeClientWithNewRegisteredAndLoggedInUser()

            await catchErrorFrom(async () => {
                await updateTestUser(client, client.user.id, {
                    password: client.user.name + faker.internet.password(12),
                })
            }, ({ errors }) => {
                expect(errors).toHaveLength(1)
                expect(errors[0]).toEqual(expect.objectContaining({
