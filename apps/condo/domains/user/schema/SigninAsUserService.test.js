...
const { makeClientWithSupportUser, signinAsUserByTestClient, makeClientWithNewRegisteredAndLoggedInUser } = require('@condo/domains/user/utils/testSchema')
...
describe('SigninAsUserService', () => {
    ...
    describe('User with direct access (canExecuteSigninAsUser)', () => {
        it('can signin as a simple user', async () => {
            const userWithDirectAccess = await makeClientWithNewRegisteredAndLoggedInUser({
                rightsSet: {
                    create: {
                        ...
                        canExecuteSigninAsUser: true,
                    },
                },
            })
            const userClient = await makeLoggedInClient()
            await signinAsUserByTestClient(userWithDirectAccess, userClient.user.id)
            const { data: { user } } = await userWithDirectAccess.query(GET_MY_USERINFO)
            expect(user.id).toEqual(userClient.user.id)
        })
        it('can not signin as a support user', async () => {
            const userWithDirectAccess = await makeClientWithNewRegisteredAndLoggedInUser({
            ...
            const supportClient = await makeClientWithSupportUser()
            await expectToThrowGQLErrorToResult(
                async () => await signinAsUserByTestClient(userWithDirectAccess, supportClient.user.id),
                {
                    mutation: 'signinAsUser',
                    code: 'FORBIDDEN',
                    type: 'DENIED_FOR_SUPPORT',
        ...
        it('can not signin as an admin user', async () => {
            ...
            await expectToThrowGQLErrorToResult(
                async () => await signinAsUserByTestClient(userWithDirectAccess, adminClient.user.id),
                {
                    mutation: 'signinAsUser',
                    code: 'FORBIDDEN',
                    type: 'DENIED_FOR_ADMIN',
