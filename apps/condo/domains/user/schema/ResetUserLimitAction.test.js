    ...
    describe('Access', () => {
        describe('Create', () => {
            test('admin: can create', async () => {
                const [obj] = await createTestResetUserLimitAction(admin, USER_AUTH_LIMIT_TYPE, phone)

                expect(obj.id).toBeDefined()
            })

            test('support: can create', async () => {
                const [obj] = await createTestResetUserLimitAction(support, USER_AUTH_LIMIT_TYPE, phone)

                expect(obj.id).toBeDefined()
            })

            test('user: can create with access right (canManageResetUserLimitActions)', async () => {
                const [obj] = await createTestResetUserLimitAction(userWithDirectAccess, USER_AUTH_LIMIT_TYPE, phone)
            ...
            test('user: can not create without access right (canManageResetUserLimitActions)', async () => {
                await expectToThrowAccessDeniedErrorToObj(async () => {
                    await createTestResetUserLimitAction(userWithoutDirectAccess, USER_AUTH_LIMIT_TYPE, phone)
                })
            })

            test('anonymous: can not execute', async () => {
                await expectToThrowAuthenticationErrorToObj(async () => {
                    await createTestResetUserLimitAction(anonymous, USER_AUTH_LIMIT_TYPE, phone)
                })
            })
        })
        ...
        describe('Read', () => {
            ...
            beforeAll(async () => {
                ...
                await redisGuard.incrementDayCounter(`:${USER_AUTH_LIMIT_TYPE}:${phone}`)

                const [createdResetUserLimitAction] = await createTestResetUserLimitAction(admin, USER_AUTH_LIMIT_TYPE, phone)
        ...
        describe('Common', () => {
            test('throws error if key is not exists', async () => {
                ...
                await expectToThrowGQLError(async () => {
                    await createTestResetUserLimitAction(admin, USER_AUTH_LIMIT_TYPE, phone)
                }, ERRORS.KEY_NOT_FOUND)
            })
        })

        describe(`${USER_AUTH_LIMIT_TYPE} type`, () => {
            test('reset counter by phone number', async () => {
                const key = `:${USER_AUTH_LIMIT_TYPE}:${phone}`

                await createTestResetUserLimitAction(userWithDirectAccess, USER_AUTH_LIMIT_TYPE, phone)
                const value = await redisGuard.getCounterValue(key)
            ...
            test('reset counter by ip', async () => {
                ...
                const key = `:${USER_AUTH_LIMIT_TYPE}:${ip}`
                ...
                await createTestResetUserLimitAction(userWithDirectAccess, USER_AUTH_LIMIT_TYPE, ip)
            ...
            test('throws error if key is not valid ip or phone', async () => {
                ...
                await expectToThrowGQLError(async () => {
                    await createTestResetUserLimitAction(userWithDirectAccess, USER_AUTH_LIMIT_TYPE, key)
