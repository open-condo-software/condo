...
describe('ResetUserLimitAction', () => {
    ...
    describe('Logic', () => {
        ...
        describe(`${CHANGE_TWO_FACTOR_AUTHENTICATION_TYPE} type`, () => {
            test('throws error if key is not exists', async () => {
                await expectToThrowGQLError(async () => {
                    await createTestResetUserLimitAction(admin, CHANGE_TWO_FACTOR_AUTHENTICATION_TYPE, faker.internet.ipv4())
                }, ERRORS.KEY_NOT_FOUND)

                await expectToThrowGQLError(async () => {
                    await createTestResetUserLimitAction(admin, CHANGE_TWO_FACTOR_AUTHENTICATION_TYPE, faker.datatype.uuid())
                }, ERRORS.KEY_NOT_FOUND)
            })

            describe('throws error if key is not valid ip, uuid', () => {
                const cases = [
                    createTestPhone(),
                    faker.random.alphaNumeric(10),
                    createTestEmail(),
                ...
                test.each(cases)('%p', async (key) => {
                    await expectToThrowGQLError(async () => {
                        await createTestResetUserLimitAction(userWithDirectAccess, CHANGE_TWO_FACTOR_AUTHENTICATION_TYPE, key)
                    }, ERRORS.INVALID_IDENTIFIER)
                })
            })
            ...
            test.each(cases)('resets rate-limit by %p', async (_, identifierKey, identifier) => {
                const dailyKey = [CHANGE_TWO_FACTOR_AUTHENTICATION_TYPE, 'daily', identifierKey, identifier].join(':')
                ...
                await createTestResetUserLimitAction(userWithDirectAccess, CHANGE_TWO_FACTOR_AUTHENTICATION_TYPE, identifier)
                const afterReset = await redisGuard.getCounterValue(dailyKey)
                expect(afterReset).toBeNull()
            })
        })
