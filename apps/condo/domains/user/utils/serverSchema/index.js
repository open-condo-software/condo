...
const { AUTHENTICATE_OR_REGISTER_USER_WITH_CONFIRM_TOKEN_MUTATION } = require('@condo/domains/user/gql')
...
async function authenticateOrRegisterUserWithConfirmToken (context, data) {
    if (!context) throw new Error('no context')
    if (!data) throw new Error('no data')
    if (!data.sender) throw new Error('no data.sender')

    return await execGqlWithoutAccess(context, {
        query: AUTHENTICATE_OR_REGISTER_USER_WITH_CONFIRM_TOKEN_MUTATION,
        variables: { data: { dv: 1, ...data } },
        errorMessage: '[error] Unable to authenticateOrRegisterUserWithConfirmToken',
        dataPath: 'obj',
    })
}
