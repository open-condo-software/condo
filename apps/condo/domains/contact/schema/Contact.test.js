...
describe('Contact', () => {
    test('required fields', async () => {
        ...
        const [obj, attrs] = await createTestContact(adminClient, userClient.property, emptyFields)
    ...
    describe('normalization', async () => {
        it('converts phone to E.164 format without spaces', async () => {
            ...
            const [obj] = await createTestContact(adminClient, userClient.property, fields)
    ...
    describe('Create', () => {
        it('can be created by admin', async () => {
            const userClient = await makeClientWithProperty()
            const adminClient = await makeLoggedInAdminClient()
            const [obj, attrs] = await createTestContact(adminClient, userClient.property)
        ...
        it('can be created by user, who is employed in organization, which manages associated property and has "canManageContacts" ability', async () => {
            const adminClient = await makeLoggedInAdminClient()
            ...
            const [obj, attrs] = await createTestContact(userClient, property)
        ...
        it('cannot be created by user, who is employed in organization, which manages associated property and does not have "canManageContacts" ability', async () => {
            const adminClient = await makeLoggedInAdminClient()
            const userClient = await makeClientWithNewRegisteredAndLoggedInUser()
            const [organization] = await createTestOrganization(adminClient)
            const [property] = await createTestProperty(adminClient, organization)
            ...
            await expectToThrowAccessDeniedErrorToObj(async () => {
                await createTestContact(userClient, property)
            })
        })
        ...
        it('cannot be created by user, who is not employed in organization, which manages associated property', async () => {
            ...
            await expectToThrowAccessDeniedErrorToObj(async () => {
                await createTestContact(anotherUser, user.property)
            })
        })
        ...
        it('cannot be created by anonymous', async () => {
            ...
            await expectToThrowAccessDeniedErrorToObj(async () => {
                await createTestContact(anonymous, userClient.property)
            })
        })
    })
    ...
    describe('Read', async () => {
        it('can be read by admin', async () => {
            const userClient = await makeClientWithProperty()
            const adminClient = await makeLoggedInAdminClient()
            const [obj] = await createTestContact(adminClient, userClient.property)
        ...
        it('cannot be read by user, who is not employed in organization, which manages associated property', async () => {
            ...
            const adminClient = await makeLoggedInAdminClient()
            const [obj] = await createTestContact(adminClient, userClient.property)
            await createTestContact(adminClient, anotherUserClient.property)
            const objs = await Contact.getAll(userClient, {}, { sortBy: ['updatedAt_DESC'] })
            expect(objs.length).toEqual(1)
            expect(objs[0].id).toMatch(obj.id)
        })
        ...
        it('cannot be read by anonymous', async () => {
            ...
            await createTestContact(adminClient, userClient.property)
    ...
    describe('Update', () => {
        ...
        it('can be updated by user, who is employed in organization, which manages associated property and does have "canManageContacts" ability', async () => {
            const adminClient = await makeLoggedInAdminClient()
            const userClient = await makeClientWithNewRegisteredAndLoggedInUser()
            const [organization] = await createTestOrganization(adminClient)
            const [property] = await createTestProperty(adminClient, organization)
            ...
            const [obj] = await createTestContact(adminClient, property)
        ...
        it('cannot be updated by user, who is employed in organization, which manages associated property and does not have "canManageContacts" ability', async () => {
            const adminClient = await makeLoggedInAdminClient()
            const userClient = await makeClientWithNewRegisteredAndLoggedInUser()
            const [organization] = await createTestOrganization(adminClient)
            const [property] = await createTestProperty(adminClient, organization)
