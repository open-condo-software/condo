...
const { makeLoggedInAdminClient, makeClient, expectToThrowAccessDeniedErrorToResult, expectToThrowAuthenticationErrorToResult } = require('@open-condo/keystone/test.utils')
const { _internalSyncContactsWithResidentsForOrganizationByTestClient, createTestContact } = require('@condo/domains/contact/utils/testSchema')
...
describe('_internalSyncContactsWithResidentsForOrganizationService', () => {
    describe('Logic', () => {
        test('sync single contact', async () => {
            ...
            const createdContact = await getById('Contact', contacts[0].id)
            const userData = userClient.userAttrs
            expect(createdContact.phone).toEqual(userData.phone)
            expect(createdContact.name).toEqual(userData.name)
            expect(createdContact.email).toEqual(userData.email)
            expect(createdContact.organization).toEqual(resident.organization.id)
            expect(createdContact.property).toEqual(resident.property.id)
        })
        test('sync multiple contact', async () => {
            ...
            expect(contacts.length).toEqual(total)
            for (const contact of contacts) {
                const createdContact = await getById('Contact', contact.id)
                const userClient = userClients.find(user => user.userAttrs.phone === createdContact.phone)
