...
const convertCreateInputToWhereInput = (createInput) => {
    const { property, category, ...whereInput } =  createInput
    const propertyId = get(property, 'connect.id')
    const categoryId = get(category, 'connect.id')
    return {
...
const RegisterPaymentRuleService = new GQLCustomSchema('RegisterPaymentRuleService', {
    types: [
        ...
        {
            access: true,
            type: 'input RegisterPaymentRuleMarketPlaceScopeInput { property: PropertyRelateToOneInput }',
        },
        {
            access: true,
            type: 'input RegisterPaymentRuleBillingScopeInput { property: PropertyRelateToOneInput, category: BillingCategoryRelateToOneInput, bankAccountNumber: String, serviceIds: JSON }',
        },
        {
            access: true,
            ...
            resolver: async (parent, args, context, info, extra = {}) => {
                ...
                if (billingScopeCreateInput) {
                    const [currentScope] = await find('PaymentRuleBillingScope', {
                        ...
                        ...convertCreateInputToWhereInput(billingScopeCreateInput),
                    })
                    if (currentScope) {
                ...
                if (marketPlaceScopeCreateInput) {
                    const [currentScope] = await find('PaymentRuleMarketPlaceScope', {
                        deletedAt: null,
                        paymentRule: { context: { id: acquiringContext.id } },
                        ...convertCreateInputToWhereInput(marketPlaceScopeCreateInput),
                    })
                    if (currentScope) {
                        if (currentScope.paymentRule !== paymentRule.id) {
