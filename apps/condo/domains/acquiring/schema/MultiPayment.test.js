    ...
    describe('CRUD tests', () => {
        describe('create', () => {
            test('admin can', async () => {
                ...
                const payerClient = await makeClientWithResidentUser()
                const { billingReceipts, acquiringIntegration } = await makePayerFromClient(payerClient)
                const [multiPayment] = await createTestMultiPayment(admin, billingReceipts, payerClient.user, acquiringIntegration)
            ...
            test('support can\'t', async () => {
                ...
                await expectToThrowAccessDeniedErrorToObj(async () => {
                    await createTestMultiPayment(support, billingReceipts, payerClient.user, acquiringIntegration)
                })
            })
            test('user can\'t', async () => {
                const client = await makeClientWithResidentUser()
                const { billingReceipts, acquiringIntegration } = await makePayerFromClient(client)
                await expectToThrowAccessDeniedErrorToObj(async () => {
                    await createTestMultiPayment(client, billingReceipts, client.user, acquiringIntegration)
                })
            })
            test('anonymous can\'t', async () => {
                ...
                await expectToThrowAuthenticationErrorToObj(async () => {
                    await createTestMultiPayment(anonymousClient, billingReceipts, client.user, acquiringIntegration)
                })
            })
        })
        describe('read', () => {
            test('admin can', async () => {
                ...
                const client = await makeClientWithResidentUser()
                const { billingReceipts, acquiringIntegration } = await makePayerFromClient(client)
                await createTestMultiPayment(admin, billingReceipts, client.user, acquiringIntegration)
            ...
            describe('user', () => {
                test('user can see only it\'s own multipayments', async () => {
                    ...
                    const { billingReceipts: firstBillingReceipts, acquiringIntegration: firstAcquiringIntegration } = await makePayerFromClient(firstClient)
                    const { billingReceipts: secondBillingReceipts } = await makePayerFromClient(secondClient)
                    const [firstMultiPayment] = await createTestMultiPayment(admin, firstBillingReceipts, firstClient.user, firstAcquiringIntegration)
                    const [secondMultiPayment] = await createTestMultiPayment(admin, secondBillingReceipts, secondClient.user, firstAcquiringIntegration)
                ...
                test('integration account can see only multipayments linked to it\'s integration', async () => {
                    const admin = await makeLoggedInAdminClient()
                    const firstClient = await makeClientWithResidentUser()
                    const { billingReceipts, acquiringIntegration: firstIntegration } = await makePayerFromClient(firstClient)
                    const [multiPayment] = await createTestMultiPayment(admin, billingReceipts, firstClient.user, firstIntegration)
    ...
    describe('real-life cases', () => {
        describe('acquiring integration account updating it\'s own MultiPayments', () => {
            describe('invalid mutations (validation cases)', () => {
                test('Cannot link multipayment to non-resident user', async () => {
                    const admin = await makeLoggedInAdminClient()
                    const residentClient = await makeClientWithResidentUser()
                    const staffClient = await makeClientWithStaffUser()

                    const { billingReceipts, acquiringIntegration } = await makePayerFromClient(residentClient)
