...
const { makePayer, getRandomHiddenCard } = require('@condo/domains/acquiring/utils/testSchema')
        ...
        describe('create', () => {
            ...
            test('support can\'t', async () => {
                ...
                await expectToThrowAccessDeniedErrorToObj(async () => {
                    await createTestMultiPayment(support, billingReceipts, client.user, acquiringIntegration)
                })
            })
        ...
        describe('read', () => {
            ...
            describe('user', () => {
                test('user can see only it\'s own multipayments', async () => {
                    const admin = await makeLoggedInAdminClient()
                    const { billingReceipts: firstBillingReceipts, acquiringIntegration: firstAcquiringIntegration, client: firstClient } = await makePayer()
                    const { billingReceipts: secondBillingReceipts, client: secondClient } = await makePayer()
                ...
                test('integration account can see only multipayments linked to it\'s integration', async () => {
                    const admin = await makeLoggedInAdminClient()
                    const { billingReceipts, acquiringIntegration: firstIntegration, client } = await makePayer()
                    const [multiPayment] = await createTestMultiPayment(admin, billingReceipts, client.user, firstIntegration)
        ...
        describe('acquiring integration account updating it\'s own MultiPayments', () => {
            describe('invalid mutations (validation cases)', () => {
                test('Cannot link multipayment to non-resident user', async () => {
                    ...
                    const { billingReceipts, acquiringIntegration } = await makePayer()
