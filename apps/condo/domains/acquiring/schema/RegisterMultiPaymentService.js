    ...
    mutations: [
        {
            ...
            resolver: async (parent, args, context) => {
                ...
                if (invoices.length > 0) {
                    const foundInvoices = await find('Invoice', { deletedAt: null, id_in: map(invoices, 'id') })

                    // All invoices must be not deleted
                    if (foundInvoices.length < invoices.length) {
                        throw new GQLError({
                            ...ERRORS.DELETED_INVOICES,
                            messageInterpolation: { ids: invoices.filter(({ id }) => !_find(foundInvoices, { id })).map(({ id }) => id).join(',') },
                        }, context)
                    }
                    ...
                    const invoicesContexts = await find('InvoiceContext', { deletedAt: null, id_in: uniq(map(foundInvoices, 'context')) })
                    ...
                    // All invoices contexts must be related to the same acquiring integration
                    if (uniq(invoicesContexts.map(({ integration }) => integration)).length !== 1) {
                        throw new GQLError(ERRORS.INVOICES_CONTEXTS_HAS_DIFFERENT_INTEGRATIONS, context)
                    }
                    ...
                    for (const invoice of foundInvoices){
                        ...
                        // const billingAccountNumber = get(frozenReceipt, ['data', 'account', 'number'])
                        const feeCalculator = new FeeDistribution([{
                            recipient: 'organization',
                            percent: Big(invoiceContext.implicitFeePercent).toFixed(2),
                        }])
                        ...
                        // const paidAmount = await getPaymentsSum(context, organizationId, billingAccountNumber, period, routingNumber, bankAccount)
                        const amount = String(Big(invoice.toPay)) //.minus(Big(paidAmount)))
