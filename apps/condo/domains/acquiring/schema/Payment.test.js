...
describe('Payment', () => {
            ...
            describe('user', () => {
                describe('resident can see it\'s own payments when it\'s linked to MultiPayment', () => {
                    ...
                    test('linked', async () => {
                        ...
                        let { data: { objs: firstUserPayments } } = await Payment.getAll(firstClient, {}, {
                        ...
                        let { data: { objs: secondUserPayments } } = await Payment.getAll(secondClient, {}, {
                            sortBy: ['createdAt_DESC'],
                            first: 10,
                            raw: true,
                        })
    ...
    describe('resolving tests', () => {
        ...
        describe('distribution and splits', () => {
            test('Should fill distribution field if paying for receipt with implicit fee', async () => {
                ...
                const [receipt, receiptAttrs] = await createTestBillingReceipt(admin, billingContext, billingProperty, billingAccount, {
                ...
                const [{ multiPaymentId }] = await registerMultiPaymentForOneReceiptByTestClient(admin, { id: receipt.id }, { id: acquiringContext.id })
                const payment = await Payment.getOne(admin, { multiPayment: { id: multiPaymentId } })

                expect(payment.frozenDistribution).toEqual(expect.objectContaining(receiptAttrs.amountDistribution))
            ...
            test('Should fill distribution field if paying for invoice', async () => {
                ...
                const [invoice, invoiceAttrs] = await createTestInvoice(adminClient, organization, {
                    property: { connect: { id: property.id } },
                    unitType,
                    unitName,
                ...
                const res = await fetch(testPaymentUrl, {
                    headers: { 'feature-flags': '[["payment-link",true]]' },
                    redirect: 'manual',
                })

                expect(res.status).toBe(302)
                expect(res.headers.get('location')).toMatch(`${acquiringIntegration.hostUrl}/api/anonymous/pay`)

                const multiPaymentRegexp = new RegExp(`\\b^${acquiringIntegration.hostUrl}/api/anonymous/pay/([^?/]+)\\b`)
                const matches = multiPaymentRegexp.exec(res.headers.get('location'))
    ...
    describe('real-life cases', () => {
        ...
        test('payment link and change invoice status to paid after payment withdrawn or done', async () => {
            ...
            await updateTestPayment(adminClient, payments[0].id, { status: PAYMENT_PROCESSING_STATUS })
            await updateTestPayment(adminClient, payments[0].id, { status: PAYMENT_WITHDRAWN_STATUS })
