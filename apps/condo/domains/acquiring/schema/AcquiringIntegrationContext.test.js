...
const { getRandomImplicitFeeDistribution } = require('@condo/domains/acquiring/utils/testSchema')
    ...
    describe('CRUD tests', () => {
        describe('create', () => {
            ...
            describe('user', async () => {
                ...
                describe('Acquiring integration', () => {
                    ...
                    test('cant create context with implicitSchema, paymentsAllowedFrom, paymentsAllowedTo as acquiring integration', async () => {
                        const admin = await makeLoggedInAdminClient()
                        ...
                        await catchErrorFrom(async () => {
                            await createTestAcquiringIntegrationContext(client, organization, integration, {
                                ...
                                paymentsAllowedFrom: dayjs().toISOString(),
                                paymentsAllowedTo: dayjs().toISOString(),
                            })
                        }, (e) => {
                            const restrictedFieldErrors = e.errors[0].data.restrictedFields
                            expect(restrictedFieldErrors).toHaveLength(3)
                            expect(restrictedFieldErrors).toContain('paymentsAllowedFrom')
                            expect(restrictedFieldErrors).toContain('paymentsAllowedTo')
                            expect(restrictedFieldErrors).toContain('implicitFeeDistributionSchema')
                        })
                    })
        ...
        describe('update', () => {
            ...
            test('support can', async () => {
                ...
                expect(newContext).toBeDefined()
                expect(newContext.integration.id).toEqual(newIntegration.id)
                expect(newContext.paymentsAllowedFrom).toEqual(startDate)
                expect(newContext.paymentsAllowedTo).toEqual(endDate)
                expect(newContext.isPaymentsAllowed).toEqual(true)
                expect(newContext.implicitFeeDistributionSchema).toBeDefined()
            })
            test('cant update context with state', async () => {
                const admin = await makeLoggedInAdminClient()
                const [organization] = await registerNewOrganization(admin)
                const [billingIntegration] = await createTestBillingIntegration(admin)
                const [integration] = await createTestAcquiringIntegration(admin, [billingIntegration])
                ...
                await catchErrorFrom(async () => {
                    ...
                    const restrictedFieldErrors = e.errors[0].data.restrictedFields
                    expect(restrictedFieldErrors).toHaveLength(1)
                    expect(restrictedFieldErrors).toContain('state')
                })
            })
            describe('user', () => {
                ...
                test('can\'t update paymentsAllowedFrom, paymentsAllowedTo and implicitFee fields as service account', async () => {
                    const admin = await makeLoggedInAdminClient()
                    const [organization] = await registerNewOrganization(admin)
                    const [billingIntegration] = await createTestBillingIntegration(admin)
                    const [integration] = await createTestAcquiringIntegration(admin, [billingIntegration])
