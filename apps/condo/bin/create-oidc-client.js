...
async function main (rawArgs) {
    ...
    const [clientId, clientSecret, redirectUriRaw, postLogoutRedirectUri] = args

    let redirectUris = [redirectUriRaw]
    try {
        redirectUris = JSON.parse(redirectUriRaw)
    } catch (e) {
        // ignore
    }
    ...
    if (!clientId || !clientSecret || !redirectUris.every(uri => uri.startsWith('http'))) throw new Error(usage)
    ...
    await createOidcClient({
        ...
        redirect_uris: redirectUris, // using uri as redirect_uri to show the ID Token contents
        isEnabled: true,
