/**
 * Remove 'None' occurances in billing accounts ('None None UKSZH...' -> 'UKSZH...')
 *
 * Usage:
 *      yarn workspace @app/condo node bin/trim-account-fullNames
 */
...
function getEntityQuery (context, entity) {
    return context.adapter.knex(entity.gql.SINGULAR_FORM)
        .whereRaw('"deletedAt" is null and "fullName" like \'None%\'')
}
...
async function main () {
    ...
    log('Start trimming account fullNames for entities: BillingAccount')
    const chunkSize = 100
