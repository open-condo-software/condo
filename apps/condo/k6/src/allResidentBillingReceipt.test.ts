...
const TOTAL_RECEIPT_NUMBER = 500
const DURATION = '60s'

export const options = {
    tags: { testid: 'allResidentBillingReceipt', serverUrl: __ENV.BASE_URL },
    scenarios: {
        allResidentBillingReceipt: {
            exec: 'allResidentBillingReceipt',
...
export function setup () {
    ...
    const { unitName, address, accountNumberPrefix } = registerBillingReceipts(data)
    createProperty({ token, organizationId }, { address })
    const residentId = registerResident(data, address, unitName).json('data.obj.id')

    const serviceConsumerId = registerResidentServiceConsumers(data, residentId, accountNumberPrefix + '0').json('data.obj.id')
...
const registerBillingReceipts = (data) => {
    ...
    const response = sendAuthorizedRequest(data, {
        ...
        query: 'mutation registerBillingReceipts($data:RegisterBillingReceiptsInput!){result:registerBillingReceipts(data:$data){id property { id address addressKey }}}',
        variables: {
            data: {
...
export function allResidentBillingReceipt (data) {
    const response = allResidentBillingReceipts(data, data.residentId)
    const receiptsResponse = response.json('data.allResidentBillingReceipts') as Array<{
        id: string,
    }>

    check(response, {
        'resident billing receipts search response is 200': (res) => res.status === 200,
        'resident billing receipts data provided': () => receiptsResponse.every(e => e.id !== undefined),
    })
}
