Condo API provides two ways to work with files:

1. **Signature chain** — use a pair of `client_id` + secret and `/api/files` endpoints to exchange signed metadata strings
2. **GraphQL Upload** — direct use of the [GraphQL multipart request spec](https://github.com/jaydenseric/graphql-multipart-request-spec)
...
## Signature chain flow (recommended)
...
<Tabs labels={['TypeScript example', 'JavaScript example']}>
```typescript
...
All endpoints require authentication via session cookie (same as GraphQL API).
...
<Alert type="warning">
    This flow is deprecated and will be removed in the future. Please migrate to the new flow.
</Alert>

The legacy flow uses GraphQL Upload directly without `fileClientId`. Files are uploaded as part of the GraphQL mutation.

### Usage

<Tabs labels={['GraphQL mutation', 'cURL example']}>
```graphql
mutation CreateDocument($data: DocumentCreateInput!) {
  obj: createDocument(data: $data) {
    id
...
* No rate limiting per application
* No file sharing between apps
* Less secure (no client ID validation)
...
To migrate from legacy flow to the new flow:

1. Install `@open-condo/files` package
2. Get `fileClientId` from your application configuration
3. Replace direct GraphQL Upload with `/api/files/upload` endpoint
4. Use the returned `signature` in your GraphQL mutations instead of the file stream

Example migration:

**Before (legacy):**
```typescript
// File uploaded directly in GraphQL mutation
...
// 2. Use signature in GraphQL mutation
const result = await createDocument({
  file: {
