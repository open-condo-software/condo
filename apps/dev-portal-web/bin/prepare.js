...
async function main () {
    ...
    // SSR Proxy for self
    const { proxyId, proxySecret } =  await registerAppProxy('dev-portal-web', 'ssr')
    await updateAppEnvFile(APP_NAME, 'SSR_PROXY_CONFIG', JSON.stringify({ proxyId, proxySecret }))

    // Generic proxy for API
    const { proxyId: apiProxyId, proxySecret: apiProxySecret } = await registerAppProxy('dev-portal-api', 'dev-portal-web')
