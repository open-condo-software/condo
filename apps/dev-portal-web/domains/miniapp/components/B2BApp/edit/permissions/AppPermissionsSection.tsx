...
type AppType = GetB2BAppQuery['app']
...
export const AppPermissionsSection: React.FC<DevicePermissionsSectionProps> = ({ id }) => {
    const intl = useIntl()
    const SectionTitle = intl.formatMessage({ id: 'pages.apps.b2b.id.sections.permissions.title' })
    const PermissionColumnTitle = intl.formatMessage({ id: 'pages.apps.b2b.id.sections.permissions.table.columns.permission.title' })
    ...
    const { data, previousData } = useGetB2BAppQuery({
        variables: { id },
    })
    ...
    const updateB2BApp = useCallback(() => {
        void updateB2BAppMutation({
            variables: {
                id,
                data: { ...appPermissions, dv: 1, sender: getClientSideSenderInfo() },
            },
        })
    }, [updateB2BAppMutation, id, appPermissions])
        ...
        <Section>
            <SubSection title={SectionTitle}>
                <Row gutter={BUTTON_GUTTER}>
                    ...
                    <Col span={FULL_COL_SPAN}>
                        <Button type='primary' onClick={updateB2BApp}>{SaveActionLabel}</Button>
                    </Col>
                </Row>
            </SubSection>
        </Section>
