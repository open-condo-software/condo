...
In the 3.0.0 release, we are going to support more different key-value databases compatible with Redis.
...
that condo can use. Making subsequent acceleration by kv-storage updates impossible.
- **Absence of virtual bases in the cluster**. In the current setup, we use 1 machine with Redis 
and its virtual bases (/0, /1, etc.) on remote and local development stands. 
To ensure scalability of the system, we will use [ValKey](https://valkey.io) cluster. 
However, cluster setups have no support for virtual bases. 
Therefore, in order not to raise a separate cluster for each application locally, 
we will use single cluster with different keys namespaces.
...
To allow multiple applications to write to the same database, you need to separate their keys from each other.
...
Renaming keys will happen as part of normal migrations, so you will not have to manually migrate existing keys 
...
1) Stop running condo applications and workers to make sure nothing's being written to the kv database.
...
> but the same instruction applies to all other keystone applications as well.

### Zero downtime migration (advanced option)

This option is a bit harder to execute, but is suitable for environments 
that have SLAs and unnecessary downtime is unacceptable.
...
`REDIS_FALLBACK_CONFIG='{"enabled": true}'`.  
By doing this, you will put the redis client into fallback mode:
    1) All write commands will use prefix:  
    `await redis.set('key', 'value') // stored as "<prefix>:key"`
    2) All read commands will try to get the value with the prefix first, and if they don't find it, they will try to get it without it.
2) Deploy 3.x version. This will rename all keys without prefix to prefixed version 
during `yarn workspace @app/condo migrate` step. 
...
> since in FALLBACK mode the response **latency from Redis will be doubled** due to fallback mode.
