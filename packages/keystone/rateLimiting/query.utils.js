 ...
 * @return number
 */
function extractWhereComplexityFactor (where, scalingFactor, maxResults) {
    /** @type number */
        ...
        if (Array.isArray(fieldValue)) {
            ...
            if (fieldName === 'AND' || fieldName === 'OR') {
                return fieldValue.reduce(
                    (totalComplexity, item) => totalComplexity + extractWhereComplexityFactor(item, scalingFactor, maxResults),
                    0.0
                )
                ...
                return Math.ceil(fieldValue.length / maxResults)
            } else {
            ...
            return scalingFactor * extractWhereComplexityFactor(fieldValue, scalingFactor, maxResults)
        } else {
