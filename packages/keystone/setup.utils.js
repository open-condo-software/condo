...
function prepareDefaultKeystoneConfig (conf) {
    ...
    const hasValkeyUrl = conf.VALKEY_URL && conf.VALKEY_URL !== 'undefined'
    const hasRedisUrl = conf.REDIS_URL && conf.REDIS_URL !== 'undefined'
    if (!IS_BUILD && (hasValkeyUrl || hasRedisUrl)) {
