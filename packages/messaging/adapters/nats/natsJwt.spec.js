...
describe('NATS JWT Utilities', () => {
    ...
    describe('computePermissions', () => {
        it('computes correct PUB-gated relay permissions for user and organization', () => {
            ...
            expect(perms.pub.allow).toContain(`_MESSAGING.subscribe.user-123.${APP_PREFIX}.user.user-123.>`)
            expect(perms.pub.allow).toContain(`_MESSAGING.subscribe.user-123.${APP_PREFIX}.organization.org-123.>`)
        ...
        it('scopes user relay to own userId only', () => {
            ...
            expect(perms.pub.allow).not.toContain(`_MESSAGING.subscribe.user-other.${APP_PREFIX}.user.user-other.>`)
        })

        it('scopes organization relay to own orgId only', () => {
            const perms = computePermissions('user-abc', 'org-456')

            expect(perms.pub.allow).toContain(`_MESSAGING.subscribe.user-abc.${APP_PREFIX}.organization.org-456.>`)
            expect(perms.pub.allow).not.toContain(`_MESSAGING.subscribe.user-abc.${APP_PREFIX}.organization.org-other.>`)
        })
    })
})
