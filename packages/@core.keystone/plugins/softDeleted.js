...
const softDeleted =
    ({ deletedAtField = 'deletedAt', newIdField = 'newId' } = {}) =>
    ...
    for (const queryValue of Object.values(whereQuery)) {
        ...
        if (Array.isArray(queryValue)) {
            for (const innerQuery of queryValue) {
                if (_queryHasSoftDeletedFieldDeep(innerQuery, deletedAtField)) return true
            }
...
function applySoftDeletedFilters(access, deletedAtField, args) {
    ...
    throw new Error(`applySoftDeletedFilters() accept a boolean or a object, received ${type}.`)
}
