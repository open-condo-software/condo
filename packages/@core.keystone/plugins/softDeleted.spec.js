...
test('on simple query with deletedAt => true', () => {
    ...
    expect(softDeleted.queryHasSoftDeletedFieldDeep(simpleQueryWithDeletedAt, DELETED_AT_FIELD)).toEqual(true)
})
...
test('on simple logical query without deletedAt => false', () => {
    expect(softDeleted.queryHasSoftDeletedFieldDeep(SIMPLE_LOGICAL_QUERY, DELETED_AT_FIELD)).toEqual(false)
})
...
test('on complex logical query without deletedAt => false', () => {
    expect(softDeleted.queryHasSoftDeletedFieldDeep(COMPLEX_QUERY, DELETED_AT_FIELD)).toEqual(false)
})
