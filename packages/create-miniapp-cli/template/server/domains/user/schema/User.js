/**
 * Generated by `createschema user.User 'name:Text; isSupport:Checkbox; isAdmin:Checkbox; email:Text;'`
 */

const { userIsAdmin, userIsAdminOrIsThisItem, userIsAdminOrIsSupport, userIsThisItem } = require('@open-condo/keystone/access')
const { historical, versioned, uuided, tracked, softDeleted, dvAndSender } = require('@open-condo/keystone/plugins')
const { GQLListSchema } = require('@open-condo/keystone/schema')

const access = require('~/domains/user/access/User')

const User = new GQLListSchema('User', {
    labelField: 'id',
    schemaDoc: 'Users authorized by oidc auth',
    labelResolver: ({ name = 'noname', email = 'noemail' }) => [name, email].filter(Boolean).join(', '),
    fields: {

        name: {
            schemaDoc: 'The user\'s name',
            type: 'Text',
            isRequired: true,
            access: {
                create: userIsAdmin,
                read: true,
                update: userIsAdmin,
            },
        },

        isSupport: {
            schemaDoc: 'Whether the user support or not',
            type: 'Checkbox',
            defaultValue: false,
            access: {
                create: userIsAdmin,
                read: (args) => (userIsAdminOrIsSupport(args) || userIsThisItem(args)),
                update: userIsAdmin,
            },
        },

        isAdmin: {
            schemaDoc: 'Whether the user admin or not',
            type: 'Checkbox',
            defaultValue: false,
            access: {
                create: userIsAdmin,
                read: (args) => (userIsAdminOrIsSupport(args) || userIsThisItem(args)),
                update: userIsAdmin,
            },
        },

        email: {
            schemaDoc: 'The user\'s email',
            type: 'Text',
            access: {
                create: userIsAdmin,
                read: (args) => (userIsAdminOrIsSupport(args) || userIsThisItem(args)),
                update: userIsAdmin,
            },
        },

        password: {
            schemaDoc: 'The user\'s password',
            rejectCommon: true,
            type: 'Password',
            access: {
                create: userIsAdmin,
                read: userIsAdmin,
                update: userIsAdmin,
            },
        },

    },
    plugins: [uuided(), versioned(), tracked(), softDeleted(), dvAndSender(), historical()],
    access: {
        read: access.canReadUsers,
        create: access.canManageUsers,
        update: access.canManageUsers,
        delete: false,
        auth: true,
    },
})

module.exports = {
    User,
}
