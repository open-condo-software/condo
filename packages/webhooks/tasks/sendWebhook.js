...
async function sendWebhook (subscriptionId) {
    ...
    try {
        ...
        if (operation === WEBHOOK_OPERATIONS.CREATE) {
            where.createdAt_gte = syncedAt
            where.deletedAt = null
        } else if (operation === WEBHOOK_OPERATIONS.UPDATE) {
            where.createdAt_lt = syncedAt
            where.updatedAt_gte = syncedAt
            where.deletedAt = null
        } else if (operation === WEBHOOK_OPERATIONS.DELETE) {
            where.createdAt_lt = syncedAt
            where.deletedAt_gte = syncedAt
        } else {
            where.updatedAt_gte = syncedAt
        }
