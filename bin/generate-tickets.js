...
const loadModels = async (admin) => {
    ...
    // fix to lvl 3 after 3lvl classifier
    return
    const classifiers2 = await TicketClassifier.getAll(admin, { parent: { id_in: Object.keys(Data.classifiers.lvlOne) } })
    Data.classifiers.lvlTwo = Object.fromEntries(classifiers2.map(({ id, name, parent }) => ([ id, { id, name, parent: parent.id }])))
    const classifiers3 = await TicketClassifier.getAll(admin, { parent: { id_in: Object.keys(Data.classifiers.lvlTwo) } })
    Data.classifiers.lvlThree = Object.fromEntries(classifiers3.map(({ id, name, parent }) => ([ id, { id, name, parent: parent.id } ])))
}
...
const getProblem = () => {
    ...
    // fix to lvl 3 after 3lvl classifier
//    const result = { lvl1: null, lvl2: null, lvl3: null }
//    const start = Object.values(Data.classifiers.lvlThree)
//    result.lvl3 = start[faker.datatype.number({ min: 0, max: start.length - 1 })]
//    result.lvl2 = Data.classifiers.lvlTwo[result.lvl3.parent]
//    result.lvl1 = Data.classifiers.lvlOne[result.lvl2.parent]
//    return result
}
...
const createTickets = async () => {
    ...
    do {
        ...
        for (const _ of arr) {
            console.log('generateTicket ... ', (++counter))
